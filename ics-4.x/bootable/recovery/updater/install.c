...
static struct UbiVolumn  ubivol[]= 
{
	{"system", "system", 1},
	{"userdata","data",  2},
	{"cache", "cache", 3},
...
extern char **environ;
static int formatubifs(const char *command)
{
    pid_t pid;
...
static void ubifsFormat(int idx, int vol, const char* name) 
{
	char cmd_line[64];
	memset(cmd_line, 0, 64);
	sprintf(cmd_line, "/format.sh %d %d %s", idx, vol, name);
   	formatubifs(cmd_line);    
}
...
Value* MountFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if(!strcmp(location, "/dev/ubi1_0"))
    {
        free(location);
        location=malloc(sizeof(char)*7);
        memcpy(location,"system",6);
        location[6] = '\0';
        fprintf(stderr, "%s:MountFn mtd partition named \"%s\" \n",
                    name, location);
    }
    if (strlen(fs_type) == 0) {
...
// Begin added by TCL kevin, mail: qik@tcl.com 2015.10.28
#define PATH_BUSYBOX "/sbin/busybox"
#define GRIDUI_TO_OLD_MS801 "V8-MS80104-LF1V885.6.zip"

static int RunCmd(const char *command) {
    char *args[] = {"busybox", "sh", "-c", NULL, NULL};
...
Value* CleanDataDirFn(const char* name, State* state, int argc, Expr* argv[]) {
    char* result = NULL;
    char* fs_type;
    char* location;
    char* mount_point;
    int i=0;
    //read argvs.
    char **argv_entries  = ReadVarArgs(state, argc, argv);
    if(argv_entries==NULL)
    {
	fprintf(stderr,"kevin____error ! argv_entries==NULL!\n");
	return ErrorAbort(state, "%s() expects some args, bug got %d", name, argc);
    }
    
    fprintf(stderr,"kevin____check argc=%d\n",argc);
    while(i < argc)
    {
	fprintf(stderr,"kevin____enter i=%d\n",i);
	fprintf(stderr,"kevin____enter argv[%d]=%s\n",i,argv_entries[i]);
	i++;
	
    }
    fprintf(stderr,"kevin____finish print argvs.\n");

    
    /*if (ReadArgs(state, argv,5,&argc_in,&fs_type,&location,&mount_point,&dir_first) < 0) {
        return NULL;
    }*/
     fs_type = argv_entries[0];
     location = argv_entries[1];
     mount_point = argv_entries[2];
     fprintf(stderr, "kevin____ok, we got fs_type: %s,"
                    "location: %s, mount_point: %s \n", 
                    fs_type, location, mount_point);
    if (strlen(fs_type) == 0) {
        ErrorAbort(state, "fs_type; argument to %s() can't be empty", name);
        goto done;
    }
    ...
    //OK，here we go，delete the specific dir now.
    char cmd[1024] = {0};
    sprintf(cmd, "busybox mount");
    int ret = RunCmd(cmd);
    fprintf(stderr, "Run mount 1 return: %d\n", ret);
	
    scan_mounted_volumes();
    const MountedVolume* volume = find_mounted_volume_by_mount_point(mount_point);
    if (volume == NULL) {
        fprintf(stderr, "Start mounting...\n");
        mkdir(mount_point, 0755);
	    ...
	    fprintf(stderr, "Yes, we mount %s succed!\n",mount_point);
            result = mount_point;
	        sprintf(cmd, "busybox mount");
    		int ret = RunCmd(cmd);
    		fprintf(stderr, "Run mount 2 return: %d\n", ret);
	
        }
    }

    fprintf(stderr,"kevin_______install.c run cmd!\n");	
    sprintf(cmd, "busybox ls -a");
    ret = RunCmd(cmd);
    fprintf(stderr, "Run ls -a return: %d\n", ret);
    //start to clean dir in /data/data
    i=3; //the dir that we should not delete!
    fprintf(stderr,"kevin____start to clean.");
    fprintf(stderr,"kevin____ try to clean,but protect %s\n",argv_entries[i]);
    sprintf(cmd, "cd /data/data;busybox ls -a | busybox grep -v %s |busybox grep -v com.android.providers.settings | busybox xargs busybox rm -rvf",argv_entries[i]);
    ret = RunCmd(cmd);
    fprintf(stderr, "Run clean cmd  return: %d\n", ret);  
    //add by kevin to delete /data/app/* @2015.1.21
    //block by kevin, we can't delete this for uesr require @2015.3.19
    /*sprintf(cmd, "cd /data/app;busybox ls -a | busybox xargs busybox rm -rvf");
    ret = RunCmd(cmd);
    fprintf(stderr, "Run clean cmd in /data/app/ return: %d\n", ret); */
    free(argv_entries);
	...
	fprintf(stderr, "Ok, Found it. let's unmount %s ~\n",mount_point);
        unmount_mounted_volume(vol);
	sprintf(cmd, "busybox mount");
    	int ret = RunCmd(cmd);
   	fprintf(stderr, "Run mount at the end return: %d\n", ret);
...
Value* FormatFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if(!strcmp(location, "/dev/ubi1_0"))
    {
        ...
        memcpy(location,"system",6);
        location[6] = '\0';
        fprintf(stderr, "%s:FormatFn mtd partition named \"%s\" \n",
                    name, location);
    }
    if (strlen(fs_type) == 0) {
        ErrorAbort(state, "fs_type argument to %s() can't be empty", name);
    ...
    if (strcmp(partition_type, "MTD") == 0) {
        ...
        //vapor add for umount before erase it
        //umount(location);
		int idx;
		int ubi_vol = -1;

		struct UbiVolumn *ubiptr = NULL;
	    for (idx = 0; idx < sizeof(ubivol)/sizeof(struct UbiVolumn); idx++) {
	    	if (!strcmp(location, ubivol[idx].name)) {
	    		ubi_vol = ubivol[idx].ubi_vol;
	    		ubiptr = &ubivol[idx];
	    		break;
	    	}
	    }
	    if (ubiptr) {
		    scan_mounted_volumes();
			char   mountpt[64];
			sprintf(mountpt, "/%s", ubiptr->tagname);
			umount(mountpt);			
	    	ubifsFormat(mtd->device_index, ubi_vol, ubiptr->tagname);
	    	result = location;
	    	fprintf(stderr, "afater format ubifs file system\n");
...
Value* RetouchBinariesFn(const char* name, State* state,
    ...
    random_bits = 0x38; //vapor for test    
...
void RegisterInstallFunctions() {
    ...
    // Begin added by TCL kevin, mail: qik@tcl.com 2015.10.28
    RegisterFunction("clean_datadir",CleanDataDirFn);
    // End of TCL
