...
mtd_scan_partitions()
{
    ...
    int i;
    ssize_t nbytes;
    MtdPartition *partitions = NULL;

    if (g_mtd_state.partitions == NULL) {
        const int nump = 32;
        partitions = (MtdPartition *)malloc(nump * sizeof(*partitions));
        if (partitions == NULL) {
            errno = ENOMEM;
            return -1;
        }
        ...
        const char *filesystem, int read_only)
{
    ...
    int rv = -1;
		...
		if (strcmp(filesystem, "ubifs") == 0 )
		{
			...
			if (strcmp(partition->name, "cache") == 0)
			{
			    printf("sszhang debug : mtd_mount_partition cache\n");
				sprintf (devname,"/dev/ubi3_0");
			}
			else if (strcmp(partition->name, "userdata") == 0)
			{
			    printf("sszhang debug : mtd_mount_partition userdata\n");

				sprintf (devname,"/dev/ubi2_0");
			}
			else if (strcmp(partition->name, "perm") == 0)
			{
			    printf("sszhang debug : mtd_mount_partition perm \n");

				sprintf (devname,"/dev/ubi4_0");
			}
			else
			{
			    printf("sszhang debug : mtd_mount_partition system\n");

				sprintf (devname,"/dev/ubi1_0");
			}
		}
		else
		{
			if (partition->device_index <= 8)
			{
				sprintf(devname, "/dev/mtd/mtd%d", partition->device_index);
			}
			else
			{
				sprintf(devname, "/dev/mtd/mtdsdm%d", partition->device_index);
			}
		}
        ...
        size_t *total_size, size_t *erase_size, size_t *write_size)
{
    ...
    {
    	sprintf(mtddevname, "/dev/mtd/mtdsdm%d", partition->device_index);
    }
...
off_t mtd_erase_blocks(MtdWriteContext *ctx, int blocks)
{
	...
	if (ctx->partition->device_index <= 8)
	{
