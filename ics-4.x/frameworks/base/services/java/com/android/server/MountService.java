...
class MountService extends IMountService.Stub
    ...
    class VoldResponseCode {
		...
		//lkm
		public static final int VolumeDiskLabel                = 633;
		//lkm
    }
                ...
                try {
                    ...
                    for (String volstr : vols) {
                        ...
                        if (!state.equals(Environment.MEDIA_REMOVED)) {
                            ...
                            if (mVolumeMap.get(path) == null) {
                                StorageVolume newVolume = new StorageVolume(path,
                                        "Secondary Storage", false, false,
                                        0, false, 0);
                                addNewVolume(newVolume);
                            }
    ...
    public boolean onEvent(int code, String raw, String[] cooked) {
        if (DEBUG_EVENTS) {
            ...
            Slog.i(TAG, String.format("lkm_code {%d}", code));
        }
        if (code == VoldResponseCode.VolumeStateChange) {
            ...
            if (action != null) {
                sendStorageIntent(action, path);
			       ...
			       deleteOldVolume(mVolumeMap.get(path));
		            }
		        } 
        	}
        //lkm 
		/*change volume label charactor to utf-8 according to filesystem type tnen send volume label event
		*/
		else if(code == VoldResponseCode.VolumeDiskLabel) {

			if (DEBUG_EVENTS) Slog.i("lkm", String.format("cooked length: {%d}", cooked.length));
			if(cooked.length == 4) {
				
			    if (DEBUG_EVENTS) Slog.i("lkm", "cooked[1]: " + cooked[1]);
			    if (DEBUG_EVENTS) Slog.i("lkm", "cooked[2]: " + cooked[2]);
			    if (DEBUG_EVENTS) Slog.i("lkm", "cooked[3]: " + cooked[3]);
			    String vLabel = null;
		           String vMountPoint = null;

				   vMountPoint = cooked[1];
			
			    if(cooked[2].equals("vfat")) {

				    try {                   
                         byte[] byteArray = cooked[3].getBytes("utf8");
                         vLabel = new String(byteArray);
				...
				Intent intent = new Intent("android.intent.action.MEDIA_VOLUME_LABEL");
				intent.putExtra("volume_label", vLabel);
				intent.putExtra("volume_mounte_point", vMountPoint);
				mContext.sendBroadcast(intent);
				
		    }else {
		        Slog.e("lkm", "Get volume label message format error");
		    }
    ...
    private void notifyVolumeStateChange(String label, String path, int oldState, int newState) {
        ...
        if(path.equals("/mnt/sdcard")) {
            ...
            vs = Environment.MEDIA_UNMOUNTED;
        }
...
/*
			//lkm
			if (DEBUG_EVENTS) Slog.i("lkm","After mount volume sucessful then send 'get volume label' command");
			try {
                mConnector.doCommand(String.format("volume label %s %s", path, label));
            } catch (NativeDaemonConnectorException e) {

			   int code = e.getCode();
			   Slog.e("lkm","Get volume fail reason:"+ code);
        	}
			//lkm
   ...
   // STA extended start.
    public String getVolumeLabel(String mountPoint) {
		...
		/*
        try {
            String[] res = mConnector.doListCommand(
             "volume label "+mountPoint, VoldResponseCode.VolumeListResult);
    ...
    public String[] getSecureContainerList() {
        ...
        if (!getVolumeState(mExternalStoragePath).equals(Environment.MEDIA_MOUNTED)) {
    ...
    public String getVolumeUuid(String mountPoint) {
        ...
        try {
            ...
            String uuid[] = mConnector.doListCommand("volume uuid " + mountPoint, VoldResponseCode.VolumeListResult);			
				Slog.d(TAG, "get volume uuid = " + uuid[0]);
            if (uuid != null && uuid.length > 0)
                return uuid[0];
        } catch (Exception e) {
            Slog.e(TAG, "Error when get volume uuid");
        }
        return null;
    }
    ...
    private void addNewVolume(StorageVolume newVolume) {
        if (newVolume == null || newVolume.getPath() == null) {
            return;
        }
        synchronized(mVolumes) {
            ...
            int i;
            for (i = 1; i < size; i++) {
                if (mVolumes.get(i) == null) {
                    mVolumes.set(i,newVolume);
                    break;
                }
            }
            if (i == size) {
                mVolumes.add(newVolume);
            }
            newVolume.setStorageId(i);
            mVolumeMap.put(newVolume.getPath(),newVolume);
        }
    }
    ...
    public Parcelable[] getVolumeList() {
        synchronized(mVolumes) {
            int size = mVolumes.size();
            ArrayList<Parcelable> temp = new ArrayList<Parcelable>();
            for (int i = 0; i < size; i++) {
                if (mVolumes.get(i) != null) {
                    temp.add(mVolumes.get(i));
                }
            }

            Parcelable [] result = new Parcelable[0];
            return temp.toArray(result);
        }
    }

    private void deleteOldVolume(StorageVolume oldVolume) {
        synchronized(mVolumes) {
            int size = mVolumes.size();
            for (int i = 1; i < size; i++) {
                if (mVolumes.get(i) == oldVolume) {
                    mVolumes.set(i,null);
                    if (oldVolume != null && oldVolume.getPath() != null) {
                        mVolumeMap.remove(oldVolume.getPath());
                    }
                    break;
                }
            }
