                    ...
                    if (buffer[i] == 0) {
                        ...
                        try {
                            ...
                            if (code >= ResponseCode.UnsolicitedInformational) {
								//lkm send diffrent charactor to Mountservice accordding to volume filesystem
								if(code == ResponseCode.VolumeDiskLabel) {
									if ((tokens[2] != null) && tokens.length >= 4) {
									    if(tokens[2].equals("vfat")) {
						                     String event_fat = new String(buffer, start, i - start, "cp936");
										     event = event_fat;
									    }
								    }
