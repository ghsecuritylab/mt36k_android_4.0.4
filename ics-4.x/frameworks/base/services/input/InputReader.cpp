...
uint32_t CursorButtonAccumulator::getButtonState() const {
    ...
    if (mBtnRight) {
        ...
        result |= AMOTION_EVENT_BUTTON_BACK;//Modify by weijb to support Mouse Right as Back
    }
    if (mBtnMiddle) {
        result |= AMOTION_EVENT_BUTTON_MENU;//Modify by weijb to support Mouse Middle Key
    }
...
/***********modify by lizhyuan for mouse start********************/
    if ((rawEvent->type == EV_SYN && rawEvent->scanCode == SYN_REPORT)&&gEnableSrcoll==true) {
 /***********modify by lizhyuan for mouse end*********************/
       sync(rawEvent->when);
    }
}
