...
//add by ireadygo
void EventHub::much_initmmap(){
	int fd;
	fd = open("/tmp/tempfile",O_CREAT|O_RDWR|O_TRUNC,0777);
	if(fd < 0){
		LOGE("can't open %s, errno=%d, %s","/tmp/tempfile", errno, strerror(errno));
		return;
	}else{
		chmod("/tmp/tempfile",00777);
	}
	lseek(fd,sizeof(muchMap)*2-1,SEEK_SET);
	write(fd,"",1);
	much_map = (muchMap*)mmap(NULL,sizeof(muchMap),PROT_READ|PROT_WRITE,MAP_SHARED,fd,0);
	close(fd);
}

/*path: /dev/input/eventx
*/
int EventHub::much_eventEnable(RawEvent* event, const char* path){
	LOGD("map_state=%d,map_name=%s,path=%s",much_map->state, much_map->name, path);
	if(strcmp(much_map->name,path) == 0){
		event->deviceId = 0;
		event->type = -1;
		event->scanCode = -1;
		event->value = -1;
