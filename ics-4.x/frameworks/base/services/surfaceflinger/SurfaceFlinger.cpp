...
#define BOOT_NAME "/tmp/boot_complete"
static int mediaserver_restart_count = 0;
static void stop_android_run(void) {
    int fd;
    char value[16] = {"0"};
    char reason[] = "secondary reboot";
    
    if((fd = open(BOOT_NAME, O_RDONLY)) >= 0) {
   	 if (property_get("mtk.failed_reboot", value, NULL) > 0) {
    		if(value == NULL) {
    			LOGD("surfaceflinger , mtk failed_reboot property is empty.\n");
                close(fd);
    			return;
    		}
    		...
    		if(atoi(value) == 0) {
        	    close(fd);
    			LOGD("surfaceflinger , mtk.failed_reboot = 0 to stop.\n");
    			property_set("ctl.stop", "runtime");
    			property_set("ctl.stop", "zygote");
 	   	} else {
    			LOGD("surfaceflinger , mtk.failed_reboot = 1 to reboot.\n");
    			sync();
    			__reboot(LINUX_REBOOT_MAGIC1, LINUX_REBOOT_MAGIC2, LINUX_REBOOT_CMD_RESTART2, reason);
    		}
    	}
   } else {
  	 LOGD("surfaceflinger , the boot_complete file does not exist..\n");
  }
}



static void mark_for_stop_android_run(void) {
	int fd;
	if((fd = open(BOOT_NAME, O_WRONLY | O_CREAT)) >= 0) {
		LOGD("surfaceflinger , Create a boot complete file.\n");
		close(fd);
	}
}
...
status_t SurfaceFlinger::readyToRun()
{
    ...
    // start boot animation
   	 ...
   	 if (property_get("mtk.failed_reboot", value, NULL) > 0) {
    		if(value != NULL) {
    			LOGD("surfaceflinger , mtk failed_reboot property is not empty.\n");
                stop_android_run(); 
    		}
    	}
...
void SurfaceFlinger::handleTransactionLocked(uint32_t transactionFlags)
{
    ...
    if (transactionFlags & eTransactionNeeded) {
		...
		/** add by mtk40641
			**/
	if (mCurrentState.composing3D != mDrawingState.composing3D) {
			LOGD("handle transaction::composing 3D changed. %d -> %d", mDrawingState.composing3D, mCurrentState.composing3D);

			const int dpy = 0;
			//const int orientation = mCurrentState.orientation;
			GraphicPlane& plane(graphicPlane(dpy));
			const int composing3D = mCurrentState.composing3D;

			//if(composing3D)
			//{			
				plane.setComposing3D(/*OSD_3D_SBSHALFE_PR*/composing3D);
			//}					
			//else
				//plane.setComposing3D(/*OSD_3D_NONE*/0x00);
		}
...
/**add mtk40641*/
status_t SurfaceFlinger::setComposing3D(DisplayID dpy, int threeD)
{
    if (UNLIKELY(uint32_t(dpy) >= DISPLAY_COUNT))
        return BAD_VALUE;
	LOGW("setComposing3D: threeD: %d",int(threeD));
    Mutex::Autolock _l(mStateLock);

	mCurrentState.composing3D = threeD;

    mDirtyRegion.set(graphicPlane(0).editDisplayHardware().bounds());
...
/**add mtk40641*/
status_t GraphicPlane::setComposing3D(uint32_t layer_type)
{
    int fd = open("/dev/graphics/fb0", O_RDWR, 0);
    if (fd >= 0) {
        if (ioctl(fd, /*FBIO_SET_UIMODE*/0x4800, layer_type) == -1) {
            LOGE("[%s] FBIO_SET_UIMODE failed. errno:%d", __func__, errno);
        }
		else 
			LOGW_IF(true, "in set 3d plane after call ioctl\n");
        close(fd);
    } else {
        LOGE("[%s] open fb failed. errno:%d", __func__, errno);
    }
	return NO_ERROR;
}
//
