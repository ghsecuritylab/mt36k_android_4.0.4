...
//added by wangyoulin for auto release
	bool notifyRelease=false;
	{
		...
		if (mClient[cameraId] != 0) 
		{
			...
			if (client != 0) 
			{
				...
				{
					LOGW("CameraService::connect X (pid %d) (the same client)",
					callingPid);
					return client;
				} else 
				{
					LOGW("CameraService::connect X (pid %d) notify client to release camera (existing client).",	callingPid);
						client->handleGenericNotify(CAMERA_MSG_RELEASE_CAMERA, UNKNOWN_ERROR, 0);
						notifyRelease=true;
				}
			}else
	...
	int reconnectTime = 0;
	bool breakok=false;
	while(reconnectTime<5)
	{
		if (notifyRelease==true) 
		{
			if (mBusy[cameraId]) 
			{
//				LOGW("camera is using by other client and still busy.");
			}else
			{
				break;
			}
...
//				LOGW("camera is busy.");
	...
	if (mClient[cameraId] != 0) 
	{
		...
		if (client != 0) 
		{
			if (cameraClient->asBinder() == client->getCameraClient()->asBinder()) 
			{
				LOGE("shoud not be here the same client error again!");
				return client;
			} else 
			{
				LOGE("rejecting (release overtime of other client).");
				return NULL;
			}
		}else
		mClient[cameraId].clear();
	}
//end by wangyoulin for auto release
