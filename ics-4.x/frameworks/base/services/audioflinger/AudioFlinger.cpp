...
bool AudioFlinger::MixerThread::threadLoop()
{
	...
	//LOGI("line %d: MixerThread::threadLoop", __LINE__);
    ...
    while (!exitPending())
    {
        ...
        if (LIKELY(mixerStatus == MIXER_TRACKS_READY)) {
			//LOGI("line %d: MixerThread::threadLoop::mAudioMixer->process()", __LINE__);
            // mix buffers...
            mAudioMixer->process();
...
#ifdef DEBUG_AUDIO_DUMP
			char value[PROPERTY_VALUE_MAX];
			property_get("debug.audio.flinger01", value, "0");
			if (atoi(value)) {
				//LOGI("line: %d debug.audio.flinger01", __LINE__);
				char fileName[PATH_MAX];

				// dump Track info
				snprintf(fileName, PATH_MAX, "/tmp/flinger01.inf");
				...
				// dump audio data
				snprintf(fileName, PATH_MAX, "/tmp/flinger01.pcm");
				int fd = ::open(fileName, O_WRONLY | O_CREAT | O_APPEND);
				if (fd != -1) {
					LOGI("line: %d debug.audio.flinger01::write %d", __LINE__, mixBufferSize);
					::write(fd, mMixBuffer, mixBufferSize);
					::close(fd);
				}
			}
#endif
            int bytesWritten = (int)mOutput->stream->write(mOutput->stream, mMixBuffer, mixBufferSize);
			//LOGI("line: %d MixerThread::threadLoop::write(%d/%d)", __LINE__, bytesWritten, mixBufferSize);
            if (bytesWritten < 0) mBytesWritten -= mixBufferSize;
#ifdef _MT53XX_AUDIO            
            if (bytesWritten > 0) memset(mMixBuffer, 0, mixBufferSize); //[DTV00372468][jiaolong]
#endif            
            mNumWrites++;
            mInWrite = false;
...
status_t AudioFlinger::RecordThread::getNextBuffer(AudioBufferProvider::Buffer* buffer)
{
    ...
    if (framesReady == 0) {
        mBytesRead = mInput->stream->read(mInput->stream, mRsmpInBuffer, mInputBytes);
#ifdef _MT53XX_AUDIO
        if (mBytesRead <= 0) { //[DTV00379199][Xiling]
#else
        if (mBytesRead < 0) {
#endif
                                ...
                                uint32_t flags)
{
    ...
    audio_hw_device_t *outHwDev = NULL;
                                ...
                                uint32_t acoustics)
{
    ...
    audio_hw_device_t *inHwDev = NULL;
