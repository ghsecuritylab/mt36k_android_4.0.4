...
#if 1
    LOGE("In MediaPlayerService: call getDefaultPlayerType() !\n");
...
bool IsMp3File(const char* url, int fd, int64_t offset)
{
    ...
    if(url)
    {
        ...
        if(sz_strlen > 4)
        {
            ...
            {
                if(!strncasecmp(url, "/mnt/", 5)) //enable cmpb player to play mp3 in usb disc
                {
                    LOGI("enable cmpbplayer to play mp3 with usb disc \n");
                    return false;
                }
            }
        }
    ...
    do
    {
        ...
        read(fd, ac_tmp, 10);
        if (memcmp("ID3", ac_tmp, 3) == 0)
        {
                 /* calculate ID3v2 tag offset */
            ui4_id3_tag_offset = 0;
    ...
    if ((ac_tmp[0] != 0xFF)                    //mp3 sync byte FF FB.
        || ((ac_tmp[1] & 0xE0) != 0xE0)
    ...
    LOGI("this file is mp3 file, android default player !!\n");
    return true;
}
...
bool IsWaveFile(const char* url, int fd, int64_t offset)
{
    ...
    if(url)
    {
        ...
        if(sz_strlen > 4)
        {
            ...
            {
                if(!strncasecmp(url, "/mnt/", 5)) //enable cmpb player to play wav in usb disc
                {
                    LOGI("enable cmpbplayer to play wav with usb disc \n");
                    return false;
                }
            }
        }
...
player_type getPlayerType(int fd, int64_t offset, int64_t length)
{
    ...
    LOGI("In MediaPlayerService: call getPlayerType(1) ident = %llx !\n", ident);
    ...
    if (ident == 0x5367674f) // 'OggS'
    {
        ...
        else
        {
            LOGI("this file is ogg file, android default player !!\n");
                return OverrideStagefrightForVorbis(STAGEFRIGHT_PLAYER);
        }
    }
    ...
    else if(IsMidiFile(NULL, fd, offset))
    {
        return SONIVOX_PLAYER;
    }

    LOGI("get player type: cmpbplayer\n");
    return CMPB_PLAYER; 
    // Some kind of MIDI?
...
{
    LOGE("In MediaPlayerService: call getPlayerType(url) !\n");
	int  fd = NULL;
	char buf[20];
	long ident = 0;
    ...
    if (IsMp3File(url, 0, 0) ||
        IsWaveFile(url, 0, 0)||
        IsAmrFile(url, 0, 0)) //use android default player to play mp3 or mid or wave or amr files.
    {
        LOGI("this file is one of mp3/wav/amr file, android default player, uri = %s !!\n",url);
        return STAGEFRIGHT_PLAYER;
    }
    else if(IsMidiFile(url, 0, 0))
    {
        LOGI("this file is mid file, android default player, uri = %s !!\n",url);
        return SONIVOX_PLAYER;
    }
...
status_t MediaPlayerService::Client:: setAudioTrack(int track_num)
{
    LOGE("Client::setAudioTrack(%d)" ,track_num );
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->setAudioTrack(track_num);
 
    return OK;   
}
status_t MediaPlayerService::Client::setSubtitleTrack(int track_num)
{
    LOGE("Client::setSubtitleTrack(d%)",track_num);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->setSubtitleTrack(track_num);
 
    return OK;   
}

status_t MediaPlayerService::Client::setPlaySeamless(bool bIsSeamless)
{
    LOGE("Client::setPlaySeamless()");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->setPlaySeamless(bIsSeamless);
 
    return OK;   
}
status_t MediaPlayerService::Client::setExSubtitleURI(const char *url)
{
    LOGE("Client::setExSubtitleURI(s%)",url);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    return p->setExSubtitleURI(url);
 
    return OK;   
}

status_t MediaPlayerService::Client::getSubtitleTrackNs(int *trk_nums)
{
    LOGE("getSubtitleTrackNs");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    status_t ret = p->getSubtitleTrackNs(trk_nums);
    if (ret == NO_ERROR) {
        LOGE("[%d] getSubtitleTrackNs = %d", mConnId, *trk_nums);
    } else {
        LOGE("getSubtitleTrackNs returned %d", ret);
    }
    return ret;
...
status_t MediaPlayerService::Client::getAudioTrackNs(int *trk_nums) 
{
    LOGE("getAudioTrackNs");
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    status_t ret = p->getAudioTrackNs(trk_nums);
    if (ret == NO_ERROR) {
        LOGE("[%d] getAudioTrackNs = %d", mConnId, *trk_nums);
    } else {
        LOGE("getAudioTrackNs returned %d", ret);
    }
    return ret;
...
sp<IMemory> MediaPlayerService::decode(const char* url, uint32_t *pSampleRate, int* pNumChannels, int* pFormat)
{
    ...
    player_type playerType = STAGEFRIGHT_PLAYER; //getPlayerType(url);  for cts test
    LOGV("player type = %d", playerType);
...
sp<IMemory> MediaPlayerService::decode(int fd, int64_t offset, int64_t length, uint32_t *pSampleRate, int* pNumChannels, int* pFormat)
{
    ...
    player_type playerType = STAGEFRIGHT_PLAYER; //getPlayerType(fd, offset, length); for cts test
    LOGV("player type = %d", playerType);
