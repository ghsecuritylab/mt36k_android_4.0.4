...
status_t MediaPlayer::setExSubtitleURI(
                    const char *url)
{
    LOGE("MediaPlayer:setExSubtitleURI(%s)", url);
    Mutex::Autolock _l(mLock);
    if (mPlayer != 0) {
        return mPlayer->setExSubtitleURI(url);
     }  
     return NO_ERROR;    
}

status_t MediaPlayer::setAudioTrack(int track_num)
{
  LOGE("MediaPlayer::setAudioTrack(%d)", track_num);
  Mutex::Autolock _l(mLock);
  if (mPlayer != 0) {
        return mPlayer->setAudioTrack(track_num);
  }  
  return NO_ERROR;
    
}

status_t MediaPlayer::setSubtitleTrack(int track_num)
{
  LOGE("MediaPlayer::setSubtitleTrack(%d)", track_num);
  Mutex::Autolock _l(mLock);
  if (mPlayer != 0) {
        return mPlayer->setSubtitleTrack(track_num);
  }  
  return NO_ERROR;  
}


status_t MediaPlayer::setPlaySeamless(bool bIsSeamless)
{
  LOGE("MediaPlayer::setPlaySeamless()");
  Mutex::Autolock _l(mLock);
  if (mPlayer != 0) {
        return mPlayer->setPlaySeamless(bIsSeamless);
  }  
  return NO_ERROR;  
}

status_t MediaPlayer::getSubtitleTrackNs(int *trk_nums)
{
    int temp;
    Mutex::Autolock _l(mLock);
    ...
    if (mPlayer == 0) return INVALID_OPERATION;
        ...
        ret = mPlayer->getSubtitleTrackNs(&temp);
        *trk_nums = temp;
        LOGE("MediaPlayer getSubtitleTrackNs: %d" , temp);   
        return ret;
    
    
}

status_t MediaPlayer::getAudioTrackNs(int *trk_nums)
{
    int temp;
    Mutex::Autolock _l(mLock);
    status_t ret = NO_ERROR;
    if (mPlayer == 0) return INVALID_OPERATION;
        ...
        ret = mPlayer->getAudioTrackNs(&temp);
        *trk_nums = temp;
        LOGE("MediaPlayer getAudioTrackNs: %d" , temp);   
        return ret;
...
{
    LOGE("MediaPlayer::setVideoRect(%d,%d, %d, %d)", i4left, i4top, i4right, bottom);
    Mutex::Autolock _l(mLock);
	
    if (mPlayer == 0) 
    {
	LOGE("MediaPlayer::setVideoRect save Rect when mplay invalid");
	return NO_ERROR;
    }
