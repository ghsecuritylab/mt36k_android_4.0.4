...
ssize_t AudioTrack::write(const void* buffer, size_t userSize)
{
	...
	if (atoi(value)) {
		...
		if (fd != -1) {
			LOGI("line %d: debug.audio.track01::write %d\n", __LINE__, userSize);
...
bool AudioTrack::processAudioBuffer(const sp<AudioTrackThread>& thread)
{
    ...
    do {
        ...
        //LOGI("line %d: obtainBuffer %d", __LINE__, audioBuffer.frameCount);
		...
		if (atoi(value)) {			
			LOGI("line %d: mCbf(%d/%d)", __LINE__, writtenSize, reqSize);
			char fileName[PATH_MAX];
			...
			int fd = ::open(fileName, O_WRONLY | O_CREAT | O_APPEND);
			if (fd != -1) {
				LOGI("line: %d debug.audio.track02::write %d", __LINE__, audioBuffer.size);
		...
		//LOGI("line %d: releaseBuffer %d", __LINE__, audioBuffer.frameCount);
        releaseBuffer(&audioBuffer);
    }
    while (frames);
