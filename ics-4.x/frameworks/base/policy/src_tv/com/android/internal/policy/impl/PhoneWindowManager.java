...
// add by wjb for getmutestate
import android.media.MediaNotify;
// end
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    //Begin Added by TCL Shenzhen  TV (TVOS)  zouwenfei mail:zouwf@tcl.com
    private static boolean isVoiceAssitantKeyDown = false; 
    private static long sLastVoiceKeyPressDownTime = 0L;
    //End Added by TCL Shenzhen  TV (TVOS)  zouwenfei mail:zouwf@tcl.com
    ...
    ChangeProjectId mChangeProjectId = null;//caosm add for changeproject id
    ...
    private void interceptPowerKeyDown(boolean handled) {
        ...
        if (!handled) {
            ...
            {
		//Add by weijb to notify userstate
			try {
				mWindowManager.setPowerOff(true);
			} catch (RemoteException e) {
			}
			Log.i(TAG, "Broadcast:"+Intent.ACTION_POWEROFF_NOTIFY);
			Intent intent_poweroff = new Intent(Intent.ACTION_POWEROFF_NOTIFY);
...
//added by kangjp for mute key process
    private boolean interceptMuteKeyDown(boolean canceled) {
		Log.d(TAG, "Mute Key Down!");                    

		Intent intent_umute = new Intent(Intent.ACTION_KEYCODE_MUTE);
		mContext.sendOrderedBroadcast(intent_umute, null);
		Log.i(TAG, "============Intent.ACTION_KEYCODE_MUTE TvWIndowManager==============");

		getAudioManager().handleKeyDown(KeyEvent.KEYCODE_VOLUME_MUTE, AudioManager.USE_DEFAULT_STREAM_TYPE);
		return true;
    }
//added by kangjp for mute key process,[end]
        ...
        public void run() {
            ...
            */
			...
			try {
				...
				if(sService != null) {
					Log.i(TAG, "------Mute Audio---------");
					sService.setStreamMute(AudioManager.STREAM_MUSIC, true, mICallBack);
				}
			} catch (Exception e) {     
			}
    ...
    public void init(Context context, IWindowManager windowManager,
	 ...
	 /**caosm add for change project id***/
	mChangeProjectId = new ChangeProjectId(mContext);
    ...
    public void setInitialDisplaySize(int width, int height) {
        ...
        mStatusBarCanHide = true;//Add by weijb to remove status bar
	...
	mHasNavigationBar = false;//Add by weijb to remove Navigation bar
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        // modify code, don't let voice assitant key pass to application, 2012-03-30
        if (keyCode == KeyEvent.KEYCODE_8 && down && event.isCtrlPressed() && event.isAltPressed()) {
            Log.d("PhoneWindowManager-kofi", "voice assitant key is down");
        ...
        if ((flags & KeyEvent.FLAG_FALLBACK) == 0) {
            ...
            if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN
				...
				//Bengin Added by TCL Xi'an  weijb Mail:weijiangbo@tcl.com
				
						final String blToken = "sys.screen.blacklight";
						if (SystemProperties.get(blToken).equals("off")) {	 
								Intent i = new Intent("com.tcl.backlight.on");
								mContext.sendBroadcast(i);
		            Slog.d(TAG,"turn on the blacklight!" + " sys.screen.blacklight= " 
		                                                       + SystemProperties.get(blToken));
        ...
        // timeout.
        if ((keyCode == KeyEvent.KEYCODE_HOME || keyCode == KeyEvent.KEYCODE_MOVE_HOME )&& getEnableHomeKey()) { //pengpai
			...
			//modification by TCL 2012-10-17
			//if the "app" is keyguard, give it the key
			try {
				if(mWindowManager.isAppKeyguard()) {
					return 0;
				}
			} catch (RemoteException e) {
	...
	boolean isTvSystemKey(int keycode)
	{
		if (keycode == KeyEvent.KEYCODE_MTKIR_PIPPOP ||
			...
			keycode == KeyEvent.KEYCODE_MTKIR_BLUE    ||
	...
	//add by zhong
	public void StartTwoDimensionCode( KeyEvent event)
	{
		boolean down = event.getAction() == KeyEvent.ACTION_DOWN;
		Log.i("*LSD*","event.value"+event.getAction());

		if(down){
			Log.i("*LSD*","StartTwoDimensionCode "+"keycode="+event.getKeyCode()+" scancode="+event.getScanCode());

// add for diandu volume operation
int scanCode = event.getScanCode();
if(scanCode ==52007 || scanCode ==  52008 ||scanCode == 52010 || scanCode == 52012 || scanCode == 52014 || scanCode == 52016 || scanCode ==52018 || scanCode ==  52020 || scanCode == 52023 || scanCode == 52025)
{
getAudioManager();
switch(scanCode)
{
  case 52007: // volume up
  mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC, mAudioManager.getStreamVolume(AudioManager.STREAM_MUSIC) + 1, AudioManager.FLAG_SHOW_UI);
		break;
  case 52008: // volume down
  mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC, mAudioManager.getStreamVolume(AudioManager.STREAM_MUSIC) - 1, AudioManager.FLAG_SHOW_UI);
  	break;
   case 52010: // set volume to 5
  	mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC, 5, AudioManager.FLAG_SHOW_UI);
	break;
  case 52012: // set volume to 10
  	mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC, 10, AudioManager.FLAG_SHOW_UI);
	break;
  case 52014: // set volume to 20
  	mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC, 20, AudioManager.FLAG_SHOW_UI);
	break;
 case 52016: // set volume to 30
  	mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC, 30, AudioManager.FLAG_SHOW_UI);
	break;
 case 52018: // set volume to 40
  	mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC, 40, AudioManager.FLAG_SHOW_UI);
	break;
case 52020: // set volume to 60
	mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC, 60, AudioManager.FLAG_SHOW_UI);
	break;
case 52023: // set volume to 80
	mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC, 80, AudioManager.FLAG_SHOW_UI);
	break;
case 52025: // set volume to 100
	mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC, 100, AudioManager.FLAG_SHOW_UI);
	break;
	default:
		break;
			...
			Intent intent1 = new Intent("com.tcl.tv.diandu");
			String name = "keycode";
			int keycode = event.getKeyCode();
			intent1.putExtra(name, keycode);
			String cmdname = "cmd";
			int cmd = event.getScanCode();
			intent1.putExtra(cmdname, cmd);
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags, boolean isScreenOn) {
        ...
        if (keyCode == KeyEvent.KEYCODE_POWER) {
            ...
            if(!isStartUi)
                {
                    Slog.d(TAG, "----------fromui------------isStartUi is false");
                    SystemProperties.set("persist.sys.bootfromcyui","false");
                }else{
                Slog.d(TAG, "----------fromui------------isStartUi is true");
                SystemProperties.set("persist.sys.bootfromcyui","true");
            }
        }
		...
		if(((keyCode & 0x0000FFFF ) & 0x1000) == 0x1000)//modify by caosm
		{
			StartTwoDimensionCode(event);
		}
        ...
        // Handle special keys.
        Log.i(TAG, "Handle special keys: " + keyCode + " \n");
        
         if (down) {
				   mChangeProjectId.inputKeyCode(keyCode);
				   }
        
        // add by zhanghangzhi for mute
        if (keyCode == KeyEvent.KEYCODE_VOLUME_UP || 
            ...
            Log.d(TAG, "[ESTALENT] keycode: " + keyCode + ", down: " + down);
            if (down) {
				   
                /*
                MtkViewHandler mMtkViewHandler = MtkViewHandler.getInstance(mContext);
                if (mMtkViewHandler != null) {
                    try {
                        mMtkViewHandler.onPreKeyDown(event);
                    } catch(Exception e) {
                        e.printStackTrace();
                    }
                }
         ...
         // add for isFocusTV
            ...
            try { 
                ...
                if (mWindowManager.isFocusTV()) {
                    Log.d(TAG, "[ESTALENT] isFocusTV, keycode: " + keyCode + ", down: " + down);
                    if (down) {
                    MtkViewHandler mMtkViewHandler = MtkViewHandler.getInstance(mContext);
                        if (mMtkViewHandler != null) {
                            try {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_MUTE: {
                if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) {
                    if (down) {
                    	//Add by weijb to support 3rd APP
				                Log.d(TAG, "========!Volume Down ========!");
												//Add by weijb for KEYCODE_VOLUME_DOWN
												Intent intent_volume_down = new Intent(Intent.ACTION_KEYCODE_VOLUME_DOWN);
												mContext.sendOrderedBroadcast(intent_volume_down, null);
												Log.i(TAG, "============Intent.KEYCODE_VOLUME_DOWN TvWIndowManager==============");
                    ...
                    if (down) {
                    	...
                    	Log.d(TAG, "========!Volume Up ========!");
											//Add by weijb for KEYCODE_VOLUME_UP
											Intent intent_voluem_up = new Intent(Intent.ACTION_KEYCODE_VOLUME_UP);
											mContext.sendOrderedBroadcast(intent_voluem_up, null);
											Log.i(TAG, "============Intent.ACTION_KEYCODE_VOLUME_UP TvWIndowManager==============");
							        getAudioManager().handleKeyDown(keyCode, AudioManager.USE_DEFAULT_STREAM_TYPE);
                ...
                if (down) {
                    ...
                    if (isMusicActive() && (result & ACTION_PASS_TO_USER) == 0) {
                        ...
                        //handleVolumeKey(AudioManager.STREAM_MUSIC, keyCode); // delete by liuwei03@tcl.com
                        break;
                    }
                }
                break;
    ...
    public void systemReady() {
        ...
        /*
        try {
			boolean initMuteFlag = MediaNotify.tvGetMuteState();
			...
			//Log.d(TAG, "[ESTALENT] initMuteFlag: " + initMuteFlag);
			if (initMuteFlag || mVolume <= 0) {
				MtkViewHandler mMtkViewHandler = MtkViewHandler.getInstance(mContext);
				if (mMtkViewHandler != null) {
					int INIT_KEYCODE = 1208;
					KeyEvent event = new KeyEvent(KeyEvent.ACTION_DOWN, INIT_KEYCODE);			
                    mMtkViewHandler.onPreKeyDown(event);
				}
			}
		} catch(Exception e) {
	...
	//modification by TCL 2012-10-17
	void startTVOrSanDUI() {
		boolean bShowSanDUI = false;
		Intent mIntent = new Intent();
		try {
			//if home key press is ignoring, ignore the keycode
			if(!getEnableHomeKey() || 
                                !mWindowManager.isHomeKeyAvailable()||
				mWindowManager.isHomeKeyPressIgnore()) {
                                Log.d(TAG, "ignore HomeKey");
				return;
			} else {
				bShowSanDUI = mWindowManager.isShowSanDUI();
			}
			
			if(bShowSanDUI) {
				mIntent.setClassName("com.tcl.tv","com.tcl.tv.TVActivity");	
			} else {
				//mIntent.setClassName("com.tcl.UI","com.tcl.UI.SanDUI");
    ...
    //start set home key pengpai 2016.1.20
    public void setEnableHomeKey(boolean bEnableHomeKey){
	try {
	    mWindowManager.setEnableHomeKey(bEnableHomeKey);
	} catch (RemoteException e) {
	}
    }

	public boolean getEnableHomeKey(){
		boolean bEnableHomeKey=true;
	
		try {
			bEnableHomeKey=mWindowManager.getEnableHomeKey();
		} catch (RemoteException e) {
			bEnableHomeKey=true;
		}
		Log.d(TAG,"TvWindowManager.bEnableHomeKey():"+bEnableHomeKey);
		return bEnableHomeKey;
	}
		//end set home key pengpai 2016.1.20
