...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
    ...
    protected boolean onKeyDown(int featureId, int keyCode, KeyEvent event) {
        ...
        Log.i(TAG, "keycode:" + keyCode + "Key down: repeat=" + event.getRepeatCount()
               + " flags=0x" + Integer.toHexString(event.getFlags()));
		try {
			if(WindowManagerHolder.sWindowManager.isFocusTV()) {
				if(keyCode == KeyEvent.KEYCODE_DPAD_RIGHT) {
					Intent intent_voluem_up = new Intent(Intent.ACTION_KEYCODE_VOLUME_UP);
					getContext().sendOrderedBroadcast(intent_voluem_up, null);
					...
					getContext().sendOrderedBroadcast(intent_volume_down, null);
					getAudioManager().handleKeyDown(keyCode, mVolumeControlStreamType);
					return true;
				}
			}
        ...
        switch (keyCode) {
	     ...
	     case KeyEvent.KEYCODE_TCL_FASTMOVE_RIGHT_1:
	     case KeyEvent.KEYCODE_TCL_FASTMOVE_RIGHT_2:
	     case KeyEvent.KEYCODE_TCL_FASTMOVE_RIGHT_3:
	     case KeyEvent.KEYCODE_TCL_FASTMOVE_RIGHT_4:
			{
	     ...
	     case KeyEvent.KEYCODE_TCL_FASTMOVE_LEFT_1:
	     case KeyEvent.KEYCODE_TCL_FASTMOVE_LEFT_2:
	     case KeyEvent.KEYCODE_TCL_FASTMOVE_LEFT_3:
	     case KeyEvent.KEYCODE_TCL_FASTMOVE_LEFT_4:
			{
                // Similar code is in PhoneFallbackEventHandler in case the window
                // doesn't have one of these.  In this case, we execute it here and
                // eat the event instead, because we have mVolumeControlStreamType
	     ...
	     case KeyEvent.KEYCODE_VOLUME_MUTE: 
			...
			Log.d(TAG, "========!Mute ========!");
			//Add by weijb for ACTION_KEYCODE_UNMUTE
			Intent intent_umute = new Intent(Intent.ACTION_KEYCODE_MUTE);
			getContext().sendOrderedBroadcast(intent_umute, null);
			Log.i(TAG, "============Intent.ACTION_KEYCODE_MUTE TvWIndowManager==============");
			//mBroadcastService.setMute(false);
			getAudioManager().handleKeyDown(keyCode, mVolumeControlStreamType);
			}
    ...
    private final class DecorView extends FrameLayout implements RootViewSurfaceTaker {
        ...
        public DecorView(Context context, int featureId) {
            ...
            //liyi.li: the floating UI sometimes displays incorrectly.
            addOnLayoutChangeListener(new OnLayoutChangeListener() {
                @Override
                public void onLayoutChange(View v, int l, int t, int r, int b,
                        int oldl, int oldt, int oldr, int oldb) {
                    if (b == oldb) {
                        Log.d(TAG, "~~~~forced layout~~~~mIsFloating=" + mIsFloating);
                        if (mIsFloating) {
                            invalidate();
                        }
                    }
