...
namespace android {
    ...
    static jstring android_net_ethernet_getMacAddressCommand(JNIEnv* env, jobject clazz)
    {
	 ...
	 int test; 	
        ...
        if(ret == -1 )
        {	     
           LOGD(">>> DRIVER MACADDR get mac addr return error!\n");
           return env->NewStringUTF(NULL);
           //return -1;
        }
        sprintf(reply ,"%02x:%02x:%02x:%02x:%02x:%02x",_mac[0],_mac[1],_mac[2],_mac[3],_mac[4],_mac[5]);
        //strncpy(reply, _mac,MAC_LEN);
        //strncpy(buf, reply,sizeof(buf));
        LOGD(">>>>>> DRIVER MACADDR --> mac = %s\n",reply);
        ...
        {
        //    LOGD(">>>>>> DRIVER MACADDR after scan --> mac = %s\n",buf);
