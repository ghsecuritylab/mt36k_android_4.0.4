...
static int doP2pCommand(const char *cmd, char *replybuf, int replybuflen)
{	
	LOGD ("doP2pCommand cmd=%s", cmd);
    size_t reply_len = replybuflen - 1;

    if (::wifi_p2p_command(cmd, replybuf, &reply_len) != 0)
        return -1;
    else {
        // Strip off trailing newline
        if (reply_len > 0 && replybuf[reply_len-1] == '\n')
...
static jint doP2pIntCommand(const char* fmt, ...)
{
    char buf[BUF_SIZE];
    va_list args;
    va_start(args, fmt);
    ...
    char reply[BUF_SIZE];
    if (doP2pCommand(buf, reply, sizeof(reply)) != 0) {
        return -1;
    }
    return static_cast<jint>(atoi(reply));
}
...
static jboolean doP2pBooleanCommand(const char* expect, const char* fmt, ...)
{
    char buf[BUF_SIZE];
    va_list args;
    va_start(args, fmt);
...
static jstring doP2pStringCommand(JNIEnv* env, const char* fmt, ...) {
    char buf[BUF_SIZE];
    va_list args;
    va_start(args, fmt);
    int byteCount = vsnprintf(buf, sizeof(buf), fmt, args);
...
{
	LOGD ("android_net_wifi_isDriverLoaded\n");
    return (jboolean)(::is_wifi_driver_loaded() == 1);
}

static jboolean android_net_wifi_loadDriver(JNIEnv* env, jobject)
{
	LOGD ("android_net_wifi_loadDriver\n");
    return (jboolean)(::wifi_load_driver() == 0);
}

static jboolean android_net_wifi_unloadDriver(JNIEnv* env, jobject)
{
	LOGD ("android_net_wifi_unloadDriver\n");
    return (jboolean)(::wifi_unload_driver() == 0);
}

static jboolean android_net_wifi_startSupplicant(JNIEnv* env, jobject)
{
	LOGD ("android_net_wifi_startSupplicant\n");
    return (jboolean)(::wifi_start_supplicant() == 0);
}

static jboolean android_net_wifi_startP2pSupplicant(JNIEnv* env, jobject)
{
	LOGD ("android_net_wifi_startP2pSupplicant\n");
    return (jboolean)(::wifi_start_p2p_supplicant() == 0);
}

static jboolean android_net_wifi_stopSupplicant(JNIEnv* env, jobject)
{
	LOGD ("android_net_wifi_stopSupplicant\n");
    return doBooleanCommand("OK", "TERMINATE");
}

static jboolean android_net_wifi_stopP2pSupplicant(JNIEnv* env, jobject)
{
	LOGD ("android_net_wifi_stopP2pSupplicant\n");

    return doP2pBooleanCommand("OK", "TERMINATE");
}

static jboolean android_net_wifi_killSupplicant(JNIEnv* env, jobject)
{
	LOGD ("android_net_wifi_killSupplicant\n");
    return (jboolean)(::wifi_stop_supplicant() == 0);
}

static jboolean android_net_wifi_killP2pSupplicant(JNIEnv* env, jobject)
{
	LOGD ("android_net_wifi_killP2pSupplicant\n");
    return (jboolean)(::wifi_stop_p2p_supplicant() == 0);
}
...
static jboolean android_net_wifi_connectToP2pSupplicant(JNIEnv* env, jobject)
{
	LOGD ("android_net_wifi_connectToP2pSupplicant\n");
	return (jboolean)(::wifi_connect_to_p2p_supplicant() == 0);
}
...
static void android_net_wifi_closeP2pSupplicantConnection(JNIEnv* env, jobject)
{
	LOGD ("android_net_wifi_closeP2pSupplicantConnection\n");
	::wifi_close_p2p_supplicant_connection();
}
static jstring android_net_wifi_waitForEvent(JNIEnv* env, jobject)
{
	LOGD ("android_net_wifi_waitForEvent\n");
...
static jstring android_net_wifi_waitForP2pEvent(JNIEnv* env, jobject)
{
    char buf[BUF_SIZE];

    int nread = ::wifi_wait_for_p2p_event(buf, sizeof buf);
    if (nread > 0) {
		LOGD ("[buf=%s]\n", buf);
        return env->NewStringUTF(buf);
    } else {
...
{
	LOGD ("android_net_wifi_doBooleanCommand\n");
...
static jboolean android_net_wifi_doP2pBooleanCommand(JNIEnv* env, jobject, jstring javaCommand)
{
    ScopedUtfChars command(env, javaCommand);
    if (command.c_str() == NULL) {
        return JNI_FALSE;
    }
    return doP2pBooleanCommand("OK", "%s", command.c_str());
}
...
static jint android_net_wifi_doP2pIntCommand(JNIEnv* env, jobject, jstring javaCommand)
{
    ScopedUtfChars command(env, javaCommand);
    if (command.c_str() == NULL) {
        return -1;
    }
    return doP2pIntCommand("%s", command.c_str());
}
...
static jstring android_net_wifi_doP2pStringCommand(JNIEnv* env, jobject, jstring javaCommand)
{
    ScopedUtfChars command(env, javaCommand);
    if (command.c_str() == NULL) {
        return NULL;
    }
    return doP2pStringCommand(env, "%s", command.c_str());
}


//start dhcpd server

static void android_net_wifi_start_dhcpd (JNIEnv* env, jobject)
{
	wifi_start_dhcpd ();
}

//stop dhcpd server
static void android_net_wifi_stop_dhcpd(JNIEnv* env, jobject)
{
	wifi_stop_dhcpd ();
}
...
static JNINativeMethod gWifiMethods[] = {
	...
	{ "stopP2pSupplicant", "()Z", (void*) android_net_wifi_stopP2pSupplicant },
    { "killSupplicant", "()Z",  (void *)android_net_wifi_killSupplicant },
	{ "killP2pSupplicant", "()Z",	(void *)android_net_wifi_killP2pSupplicant },
    { "connectToSupplicant", "()Z",  (void *)android_net_wifi_connectToSupplicant },
	{ "connectToP2pSupplicant", "()Z",  (void *)android_net_wifi_connectToP2pSupplicant },
    { "closeSupplicantConnection", "()V",  (void *)android_net_wifi_closeSupplicantConnection },
	{ "closeP2pSupplicantConnection", "()V",  (void *)android_net_wifi_closeP2pSupplicantConnection },
	...
	{ "waitForP2pEvent", "()Ljava/lang/String;", (void*) android_net_wifi_waitForP2pEvent },
    ...
    { "doP2pBooleanCommand", "(Ljava/lang/String;)Z", (void*) android_net_wifi_doP2pBooleanCommand},
    { "doIntCommand", "(Ljava/lang/String;)I", (void*) android_net_wifi_doIntCommand},
	{ "doP2pIntCommand", "(Ljava/lang/String;)I", (void*) android_net_wifi_doP2pIntCommand},
    { "doStringCommand", "(Ljava/lang/String;)Ljava/lang/String;", (void*) android_net_wifi_doStringCommand},
//    { "wifiStartDhcpd", "()V", (void*) android_net_wifi_start_dhcpd},
//    { "wifiStopDhcpd", "()V", (void*) android_net_wifi_stop_dhcpd},
    { "doP2pStringCommand", "(Ljava/lang/String;)Ljava/lang/String;", (void*) android_net_wifi_doP2pStringCommand},
};
