...
//added by wang youlin
    private static final int CAMERA_MSG_RELEASE_CAMERA 	 = 0x1000;
//end by wang youlin
    private static final int CAMERA_MSG_ALL_MSGS         = 0xFFFF;
...
public        void onRelease(Camera camera);
    ...
    /**
     * Registers a callback to be invoked when an release msg occurs.
     * @param cb The callback to run
     */
    public final void setReleaseCallback(ReleaseCallback cb)
    {
        mReleaseCallback = cb;
    }
//end by wang youlin
...
//added by wang youlin
			case CAMERA_MSG_RELEASE_CAMERA :
                if (mReleaseCallback != null) {
                    mReleaseCallback.onRelease(mCamera);
                }
//end by wang youlin
...
//added by wang youlin
		params.remove(Parameters.KEY_CONTROL_BRIGHTNESS_VALUE);
		params.remove(Parameters.KEY_CONTROL_CONTRAST_VALUE);
		params.remove(Parameters.KEY_CONTROL_SHARPNESS_VALUE);
		params.remove(Parameters.KEY_CONTROL_SATURATION_VALUE);
		params.remove(Parameters.KEY_CONTROL_WHITENESS_VALUE);
//end by wang youlin
...
//added by wang youlin
     ...
     * Control parameter .
     */
    public class ControlParameterScope {
        /**
         * Sets the dimensions for pictures.
         *
         * param ValueNow the currunt value
         * param Minimum the values of the scope
         */
        public ControlParameterScope(int ValueNow, int Minimum, int Maximum, int Step, int Defaultvalue) {
		valueNow=ValueNow;
            minimum = Minimum;
            maximum = Maximum;
            step = Step;
        ...
        @Override
        public boolean equals(Object obj) {
            if (!(obj instanceof ControlParameterScope)) {
                return false;
            }
            ControlParameterScope s = (ControlParameterScope) obj;
            return minimum == s.minimum && maximum == s.maximum && step == s.step && defaultvalue == s.defaultvalue;
        }
        @Override
        public int hashCode() {
            return minimum * 32713 + maximum;
        }
        public int valueNow;
        public int minimum;
        public int maximum;
...
//added by wang youlin
        public static final String KEY_CONTROL_BRIGHTNESS_SCOPE = "brightness-scope";
        public static final String KEY_CONTROL_BRIGHTNESS_VALUE = "brightness-value";
        public static final String KEY_CONTROL_CONTRAST_SCOPE = "contrast-scope";
        public static final String KEY_CONTROL_CONTRAST_VALUE = "contrast-value";
        public static final String KEY_CONTROL_SATURATION_SCOPE = "saturation-scope";
        public static final String KEY_CONTROL_SATURATION_VALUE = "saturation-value";
        public static final String KEY_CONTROL_SHARPNESS_SCOPE = "sharpness-scope";
        public static final String KEY_CONTROL_SHARPNESS_VALUE = "sharpness-value";
        public static final String KEY_CONTROL_WHITENESS_SCOPE = "whiteness-scope";
        public static final String KEY_CONTROL_WHITENESS_VALUE = "whiteness-value";
        private static final String KEY_INTERPOLATION_SIZE = "interpolation";
//end by wang youlin
...
//added by wang youlin
        public void setInterpolationSize(int width, int height) {
            String v = Integer.toString(width) + "x" + Integer.toString(height);
            set(KEY_INTERPOLATION_SIZE, v);
        }
        public void setControlParameter(final String Control, int Values) {
            String v = Integer.toString(Values) ;
            set(Control, v);
        }
        public ControlParameterScope getControlParameterScope(final String Control) {
		ControlParameterScope scope=new ControlParameterScope(0,0,0,0,0);
            String str = get(Control);
            if (str == null) return scope;

           StringTokenizer tokenizer = new StringTokenizer(str, ",");
		if (tokenizer.hasMoreElements()) 
		{
                scope.valueNow= Integer.parseInt(tokenizer.nextToken());
		}
 		if (tokenizer.hasMoreElements()) 
		{
                scope.minimum = Integer.parseInt(tokenizer.nextToken());
		}
 		if (tokenizer.hasMoreElements()) 
		{
                scope.maximum= Integer.parseInt(tokenizer.nextToken());
		}
 		if (tokenizer.hasMoreElements()) 
		{
                scope.step= Integer.parseInt(tokenizer.nextToken());
		}
 		if (tokenizer.hasMoreElements()) 
		{
                scope.defaultvalue= Integer.parseInt(tokenizer.nextToken());
		}
            return scope;
        }
//end by wang youlin
