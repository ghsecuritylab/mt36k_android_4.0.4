...
public final class WebViewCore {
    ...
    private static boolean mJustInited = false;
    // -- MTK_WEBKIT_FIX END --
    ...
    private void initialize() {
        ...
        if (!mJustInited) {
            mJustInited = true;
            Log.e(LOGTAG, "Clear all cache (include disk cache) first...");
            clearCache(true);
        }
        // -- MTK_WEBKIT_FIX END --
    }
    ...
    */
    ...
    private void clearCache(boolean includeDiskFiles) {
        // -- MTK_WEBKIT_FIX START --
        mBrowserFrame.clearCache(includeDiskFiles);
        // -- MTK_WEBKIT_FIX END --
        if (includeDiskFiles) {
    ...
    // -- MTK_WEBKIT_FIX START --
    // called by JNI.
    private void startEmbeddedVideoStreaming(int npp) {
        if (mWebView == null) {
            return;
        }

        Message message = mWebView.mPrivateHandler.obtainMessage(WebView.START_EMBEDDED_VIDEO_STREAMING);
        message.arg1 = npp;
        message.sendToTarget();
    }
    // -- MTK_WEBKIT_FIX END --
