...
const char CameraParameters::KEY_CONTROL_BRIGHTNESS_SCOPE[] = "brightness-scope";
const char CameraParameters::KEY_CONTROL_BRIGHTNESS_VALUE[] = "brightness-value";
const char CameraParameters::KEY_CONTROL_CONTRAST_SCOPE[] = "contrast-scope";
const char CameraParameters::KEY_CONTROL_CONTRAST_VALUE[] = "contrast-value";
const char CameraParameters::KEY_CONTROL_SATURATION_SCOPE[] = "saturation-scope";
const char CameraParameters::KEY_CONTROL_SATURATION_VALUE[] = "saturation-value";
const char CameraParameters::KEY_CONTROL_SHARPNESS_SCOPE[] = "sharpness-scope";
const char CameraParameters::KEY_CONTROL_SHARPNESS_VALUE[] = "sharpness-value";
const char CameraParameters::KEY_CONTROL_WHITENESS_SCOPE[] = "whiteness-scope";
const char CameraParameters::KEY_CONTROL_WHITENESS_VALUE[] = "whiteness-value";
const char CameraParameters::KEY_INTERPOLATION_SIZE[] = "interpolation";
//end by wang youlin
...
//added by wang youlin
void CameraParameters::getInterpolationSize(int *width, int *height) const
{
    *width = *height = -1;
    // Get the current string, if it doesn't exist, leave the -1x-1
    const char *p = get(KEY_INTERPOLATION_SIZE);
    if (p == 0)  return;
    parse_pair(p, width, height, 'x');
	
//	remove(KEY_INTERPOLATION_SIZE);
}

void CameraParameters::getControlParameterValue(const char* Control,int *Value) const
{
    *Value = -32768;
    // Get the current string, if it doesn't exist, leave the -1x-1
    const char *str = get(Control);
    if (str == 0)  return;
...
void CameraParameters::setControlParameterScope(const char* Control, int ValueNow, int Minimum, int Maximum, int step, int DefaultValue)
{
    char str[32];
    sprintf(str, "%d,%d,%d,%d,%d", ValueNow, Minimum, Maximum, step, DefaultValue);
    set(Control, str);
}
//end by wang youlin
