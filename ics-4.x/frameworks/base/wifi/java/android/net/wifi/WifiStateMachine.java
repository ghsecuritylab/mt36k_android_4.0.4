...
public class WifiStateMachine extends StateMachine {
    ...
    private CurrentBssInfo mCurrentBssInfo;

    private int mDongleType = -1;
    ...
    // sync with wifi.c multi dongle type
    private static final int INT_INTERFACE_NO_DONGLE     = -1;
    private static final int INT_INTERFACE_RALINK_5570   =  0;
    private static final int INT_INTERFACE_REALTEK_CU    =  1;
    private static final int INT_INTERFACE_REALTEK_SU    =  2;
    private static final int INT_INTERFACE_REALTEK_DU    =  3;
    private static final int INT_INTERFACE_ATHEROS_K2    =  4;
    private static final int INT_INTERFACE_ATHEROS_ATH6K =  5;
    
    private static final int RALINK_5570_MAX_RSSI = -55;
    private static final int RALINK_5570_MIN_RSSI = -100;
    
    private static final int ATHEROS_K2_MAX_RSSI = 94;
    private static final int ATHEROS_K2_MIN_RSSI = 0;
    ...
    public WifiStateMachine(Context context, String wlanInterface) {
        ...
        mCurrentBssInfo = new CurrentBssInfo();
        ...
        mContext.registerReceiver(
            new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
					...
					loge("Nat_test:sendMessage CMD_TETHER_STATE_CHANGE");
        ...
        if (line != null) {
            ...
            synchronized (mScanResultCache) {
                ...
                if (3 <= result.length && result.length <= 5) {
                    ...
                    int rssi = calcRssi(level, mDongleType);
                    log("rssi:" + rssi);
                    /*
                    ...
                    if (scanResult != null) {
                        ...
                        if (0 < ssid.trim().length()) {
                            scanResult =
                                new ScanResult(
                                    ssid, bssid, flags, rssi, frequency);
	...
	private String  fetchFrequency(){
        ...
        // extract ChannelNumber from a series of "name=value"
        ...
        for (String line : lines) {
            ...
            String name = prop[0];
            String value = prop[1];
            if (name.equalsIgnoreCase("FREQUENCY")) return value;
        }
        return null;
        
	}
	 private String fetchProtocol_caps() {
        String status = WifiNative.statusCommand();
        if (status == null) {
            return null;
        }
        ...
        for (String line : lines) {
            ...
            String name = prop[0];
            String value = prop[1];
            if (name.equalsIgnoreCase("protocol_caps")) return value;
        }
        return null;
    }
    ...
    private void handleNetworkDisconnect() {
		...
		mWifiInfo.setChannelNumber(0);						
		mWifiInfo.setFrequency(null);
		mWifiInfo.setProtocol_caps(null);
    ...
    private void handleSuccessfulIpConfiguration(DhcpInfoInternal dhcpInfoInternal) {
        ...
        if (getNetworkDetailedState() == DetailedState.CONNECTED) {
            ...
            if (null != dhcpInfoInternal.ipAddress && dhcpInfoInternal.ipAddress.contains("169.254")) {
...
*/
            ...
            switch(message.what) {
                ...
                case CMD_START_SUPPLICANT:
                   ...
                   try {
					...
					if(DBG) log("killSupplicant Before startSupplicant!");
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                ...
                case WifiMonitor.NETWORK_CONNECTION_EVENT:
                    ...
                    mWifiInfo.setFrequency(fetchFrequency());
					mWifiInfo.setChannelNumber(mWifiInfo.getChannelNumber(fetchFrequency()));
					//mWifiInfo.setGroupCipher(String GroupCipher);
					//mWifiInfo.setHiddenSSID(boolean hiddenSSID);
					//mWifiInfo.setInetAddress(InetAddress address);
					//mWifiInfo.setKeyMgmt(String KeyMgmt);
					//mWifiInfo.setLinkSpeed(int linkSpeed);
					mWifiInfo.setProtocol_caps(fetchProtocol_caps());
 ...
 //       return WifiNative.getCurrentBss();
        return "";
    }
...
//    	return WifiNative.removeNetworkCommandAll();
        return true;
    }
 ...
 //   	if(WifiNative.wifi0Check() < 0) {
//    		log("[WifiStateTracker][isDongleExist]: Dongle does not exist.");
//   		return false;
//    	}
    	
//    	log("[WifiStateTracker][isDongleExist]: Dongle exist.");
    	return true;
    }
...
//        return 2; //current is wlan0 (Ralink)
    ...
    public int calcRssi(int level, int type) {
        /*if(type == INT_INTERFACE_NO_DONGLE) {
            return 0;
        }
        if(type == INT_INTERFACE_ATHEROS_K2 || type == INT_INTERFACE_ATHEROS_ATH6K) {
            if(level >= 42) {
                return ATHEROS_K2_MAX_RSSI;
            } else if (level >= 30) {
                return 85 + ((ATHEROS_K2_MAX_RSSI-85) * (level - 30) + (42 - 30)/2)/(42 -30);
            } else if (level >= 5) {
                return 5 + ((level - 5) * (85 -5) + (30 -5)/2)/(30 -5);
            } else if (level >= 1) {
                return level;
            } else {
                return ATHEROS_K2_MIN_RSSI;
            }
        }
		else if (type == INT_INTERFACE_REALTEK_SU
				||type == INT_INTERFACE_REALTEK_CU 
				||type == INT_INTERFACE_RALINK_5570) {
                if (level > 0) 
						level -= 256;
                 if( level >= RALINK_5570_MAX_RSSI ) {
                    return 100;
                } else if( level <= RALINK_5570_MIN_RSSI ) {
                    return 0;
                } else {
                    return (int) ( ( (double)( level - RALINK_5570_MIN_RSSI)
                           /(double) (RALINK_5570_MAX_RSSI - RALINK_5570_MIN_RSSI) ) * 100 );
                }
        }*/
    ...
    private boolean requestCurrentBssInfo(CurrentBssInfo bssInfo) {
    	String currnetBss = getInfo();
    	if(currnetBss == null) {
    		log("[WifiStateTracker][requestCurrentBssInfo]:  Request Current BSS Failed.");
    		return false;
    	}
    	...
    	String Bssid = null;
    	int Netid = -1;
    	String PairwiseCipher = null;
    	String GroupCipher = null;
    	String KeyMgmt = null;
    	String WpaState = null;
    	int Rssi = 0;
    	int ChannelNumber = 0;
    	String frequency = null;
    	
        String[] lines = currnetBss.split("\n");
        for (String line : lines) {
            String[] prop = line.split(" *= *");
            if (prop.length < 2)
                continue;
            ...
            if(name.equalsIgnoreCase("bssid")) {
            	...
            	continue;
            } else if (name.equalsIgnoreCase("pairwise_cipher")) {
            	PairwiseCipher = value;
            	continue;
            } else if (name.equalsIgnoreCase("group_cipher")) {
            	GroupCipher = value;
            	continue;
            } else if (name.equalsIgnoreCase("key_mgmt")) {
            	KeyMgmt = value;
            	continue;
            	...
            	ChannelNumber = Integer.parseInt(value);
            	continue;
            } else if (name.equalsIgnoreCase("freq")) {
            	if(value != null) {
            		frequency = value.substring(0, value.indexOf("G"));
            	}
            	continue;
            }
        }
        
        log("currnet link's freqency is: " + frequency);
        
        bssInfo.setSSID(Ssid);
        bssInfo.setBSSID(Bssid);
        bssInfo.setNetworkId(Netid);
        bssInfo.setPairwiseCipher(PairwiseCipher);
        bssInfo.setGroupCipher(GroupCipher);
        bssInfo.setKeyMgmt(KeyMgmt);
        bssInfo.setWpaState(WpaState);
        bssInfo.setChannelNumber(ChannelNumber);
        bssInfo.setFrequency(frequency);
        
        int type = wifi0Check();;
        
        int level = calcRssi(Rssi, type);         
        if(type != INT_INTERFACE_NO_DONGLE) log("[WifiService][parseScanResult]: type: " + type + 
            	" level: " + Rssi	+ " calcRssi: " + level);
        
        bssInfo.setRssi(level);
       
    	return true;
    }
    
    
    public CurrentBssInfo getCurrentBssInfo() {
    	if (mWifiState.get() != WIFI_STATE_ENABLED) {
    		return null;
    	}
    	
    	if(!requestCurrentBssInfo(mCurrentBssInfo)) {
    		return null;
    	}
    	
    	return mCurrentBssInfo;
    }
}
