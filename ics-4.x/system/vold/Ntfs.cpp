...
int Ntfs::doMount(const char * fsPath, const char * mountPoint,
	...
	*/
	...
	if(value[0] == '1'){
		SLOGW("The SD card is world-writable because the 'persist.sampling_profiler' system property is set to '1'");
		permMask = 0;
	}
/*
	sprintf(mountData,"uid =%d, gid = %d,fmask = %o,dmask =%o,nls = utf8",
			ownerUid,ownerGid,permMask,permMask);
			*/
	sprintf(mountData,"nls=utf8,umask=0000");
	rc = mount(fsPath, mountPoint, "ntfs",flags,mountData);
	if((rc && errno == EROFS) || rc == -1){
		SLOGE("%s appears to be a read only filesystem --retring mount RO", fsPath);
		flags |= MS_RDONLY;
		rc = mount(fsPath, mountPoint, "ntfs",flags,mountData);
	}
