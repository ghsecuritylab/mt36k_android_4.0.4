    ...
    while(fgets(line, sizeof(line), fp)) {
        ...
        if (!strcmp(type, "dev_mount")) {
            ...
            if (strcmp(part, "auto") && atoi(part) == 0) {
			 ...
			 SLOGD("[process_config]type= %s, label= %s, mount_point= %s, part= %s.\n",
                         type, label, mount_point, part);
            ...
            while ((sysfs_path = strtok_r(NULL, delim, &save_ptr))) {
				...
				SLOGD("sysfs_path = %s",sysfs_path);
                if (dv->addPath(sysfs_path)) {
                    SLOGE("Failed to add devpath %s to volume %s", sysfs_path,
                         label);
                    goto out_fail;
                }
				SLOGD("add sysfs_path into volumeManager");
                if (vm->addPath(sysfs_path)) {
                    SLOGE("Failed to add devpath %s to volume %s", sysfs_path,
                         label);
                    goto out_fail;
                }
