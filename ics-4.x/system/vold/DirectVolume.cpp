    ...
    const char *dp = evt->findParam("DEVPATH");
		...
		/* We can handle this disk */
            ...
            if (action == NetlinkEvent::NlActionAdd) {
				...
				SLOGD("####NetlinkEvent::NlActionAdd#####");
				//lkm
				SLOGD("lkm_getState:%s",getState());
				//lkm
				...
				SLOGD("####NetlinkEvent::NlActionRemove#####");
                if (!strcmp(devtype, "disk")) {
				...
				SLOGD("####NetlinkEvent::NlActionChange#####");
                if (!strcmp(devtype, "disk")) {
                    ...
                    SLOGE("Ignoring non add/remove/change event");
            }
...
void DirectVolume::handlePartitionRemoved(const char *devpath, NetlinkEvent *evt) {
    ...
    if ((dev_t) MKDEV(major, minor) == mCurrentlyMountedKdev) {
        ...
        if (mVm->unshareVolume(getLabel(), "ums")) {
            ...
            SLOGD("Volume::State_Shared--Crisis averted");
        }
		//lkm
		setState(Volume::State_NoMedia);
		//lkm
