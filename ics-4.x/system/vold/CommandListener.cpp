...
int CommandListener::VolumeCmd::runCommand(SocketClient *cli,
    ...
    if (!strcmp(argv[1], "list")) {
        ...
        if (argc != 3) {
            cli->sendMsg(ResponseCode::CommandSyntaxError, "Usage: volume uuid <mountPoint>", false);
            return 0;
        }
        char* pUuid = vm->getVolumeUuid(argv[2]);
        if (pUuid && strlen(pUuid) > 0)
            cli->sendMsg(ResponseCode::VolumeListResult, pUuid, false);
    } else {
