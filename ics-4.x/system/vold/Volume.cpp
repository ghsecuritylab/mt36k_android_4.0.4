...
int Volume::mountVol() {
    ...
    for (i = 0; i < n; i++) {
		...
		if(isNtfsFS){
			...
			{
				SLOGI("ntfs-3g mount\n");
				if(mountVolumeByNtfs3g(devicePath, "/mnt/secure/staging")){
					SLOGE("%s failed to mount via NTFS3G (%s)\n", devicePath,strerror(errno));
					isNtfsFS = false;
				} else {
					isExtFS = false;
				}
			}		
			/*if(Ntfs::doMount(devicePath,"/mnt/secure/staging",false,false,1000,1015,0702,true)){
...
int Volume::createBindMounts() {
    ...
    if (mount("tmpfs", SEC_STG_SECIMGDIR, "tmpfs", MS_RDONLY, "size=0,mode=000,uid=0,gid=0")) {
		...
		umount2("/mnt/asec_secure", MNT_FORCE|MNT_DETACH);
        return -1;
    }
