                     ...
                     uint32_t  *lease)
{
    ...
    snprintf(prop_name, sizeof(prop_name), "%s.%s.reason", DHCP_PROP_NAME_PREFIX, interface);
    property_get(prop_name, prop_value, NULL);
    if (strcmp(prop_value, "IPV4LL") == 0) {
        ifc_add_route(interface, "0.0.0.0", 0, "0.0.0.0");
    }
                    ...
                    uint32_t  *lease)
{
    ...
    if (wait_for_property(daemon_prop_name, desired_status, 30) < 0) {
    ...
    if (wait_for_property(result_prop_name, NULL, 200) < 0) {
