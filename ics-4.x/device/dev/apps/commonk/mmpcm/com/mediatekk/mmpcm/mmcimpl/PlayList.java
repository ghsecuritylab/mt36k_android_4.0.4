...
import com.mediatekk.mmpcm.mmc.IPlayList;
	...
	private void setShuffleMode(int filterType) {
		...
		case Const.FILTER_TEXT:
			...
			if(index>=0)
			{
				...
				if(curBackIdx>=0)
				{
					tempIndex=curBackIdx;
				}
				try {
					String shufflepath=foreShuffleFileList.get(tempIndex);
					foreShuffleFileList.set(tempIndex, path);
					foreShuffleFileList.set(index, shufflepath);
				} catch (Exception e) {
