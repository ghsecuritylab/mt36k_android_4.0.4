...
import com.mediatekk.mmpcm.video.IComset;


public class Comset implements IComset {
...
private int curZoomType;
	...
	public void setZoomType(int type){
		...
		{
			MmpTool.LOG_DBG("Comset.setZoomType" + "This zoom type is not supported !!!");
			curZoomType = VideoConst.VOUT_ZOOM_TYPE_1X;
		}
		curZoomType = type;
	}	
	...
	public int videoZoom( int videoZoomType )
    {
    	...
    	orgVidRect = inputManager.getScreenOutputVideoRect( InputService.INPUT_OUTPUT_MAIN);
    	orgDspRect = inputManager.getScreenOutputRect( InputService.INPUT_OUTPUT_MAIN );
    	if(null != orgVidRect){
    	...
    	adjVidRect.left = (int) (VideoConst.VOUT_REGION_MAX_WIDTH / 2 - 
    					VideoConst.VOUT_REGION_MAX_WIDTH / fdivisor / 2);
    	adjVidRect.right = (int) (VideoConst.VOUT_REGION_MAX_WIDTH / 2 + 
						VideoConst.VOUT_REGION_MAX_WIDTH / fdivisor / 2);
    	adjVidRect.top = (int) (VideoConst.VOUT_REGION_MAX_HEIGTH / 2 - 
						VideoConst.VOUT_REGION_MAX_HEIGTH / fdivisor / 2);
    	adjVidRect.bottom = (int) (VideoConst.VOUT_REGION_MAX_HEIGTH / 2 + 
						VideoConst.VOUT_REGION_MAX_HEIGTH / fdivisor / 2);
    	
    	ret = inputManager.setScreenOutputVideoRect( InputService.INPUT_OUTPUT_MAIN, adjVidRect);
    	MmpTool.LOG_DBG("setScreenOutputVideoRect ret = " + ret );
    	if( ret == InputService.INPUT_RET_FAIL) return -1;
    	
    	ret = inputManager.setScreenOutputRect( InputService.INPUT_OUTPUT_MAIN, adjDspRect);
	...
	private int calMaxZoom()
	{
		...
		OutputRegionCapability capability=inputManager.getScreenOutputVideoCapability
									(InputService.INPUT_OUTPUT_MAIN);
		if(null != capability){
			MmpTool.LOG_INFO("video" + "capability.width_min:"+capability.width_min);
			MmpTool.LOG_INFO("video" + "capability.height_min:"+capability.height_min);	
		}
		
		for( maxZoom = VideoConst.VOUT_ZOOM_TYPE_1X;
			 maxZoom <= VideoConst.VOUT_ZOOM_TYPE_512X;
			 maxZoom++)
		{
			if( 0 == maxZoom % 2 )
	    	{
			...
			if( (capability.width_min > videoWidth) || 
				(capability.height_min > videoHeight))
			{
