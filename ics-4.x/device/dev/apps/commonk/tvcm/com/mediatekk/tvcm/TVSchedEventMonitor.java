...
import com.mediatekk.tvcm.TVEventManager.EventListener;
...
public class TVSchedEventMonitor {
	...
	TVChannel lookforChannelFromRaw(int svlId, int channelId) {
		if (channels != null) {
		for (TVChannel ch : channels) {
			if (ch.getRawInfo().getChannelId() == channelId
					&& ch.getRawInfo().getSvlId() == svlId) {
				return ch;
			}
		}
		}
	...
	void notifyCh(final TVChannel ch) {
		em.getHandler().post(new Runnable() {
			public void run() {
				if (listener != null) {
	...
	void onSchedulChange(int svlId, int channelId) {
		...
		if (ch == null) {
			TVComponent.TvLog("LookforChannelFromRaw failed!");
			return;
		}

		Map<ChannelInfo, ArrayList<EventInfo>> rawEventMap;
		try {
			rawEventMap = em.rawSrv.getScheduleEvents(new ChannelInfo[]{ch
					.getRawInfo()});
			if (rawEventMap == null) {
				TVComponent.TvLog("GetScheduleEvents failed!");
				return;
			}
			map.remove(ch);
	...
	void read(boolean notifyEach) {
		...
		if (!dummyMode) {
			...
			for (TVChannel ch : channels) {
				for (long ti = startTime; ti < startTime + duration;) {
					long du = (long) (Math.random() * 1200f * 4000f);
					addInfo(ch, new TVEvent.DummyEvent(ch, ti, du));
					ti += du;
				}
				if (notifyEach) {
					if (notifyEach) {
