package com.mediatekk.netcm.dlna;
...
public class DLNAManager implements DeviceEventListener, ContentEventListener {
	...
	class DLNATempFile {
		...
		public boolean equals(DLNATempFile fileIns) {
			...
			if (type != fileIns.getType())
				return false;
			if (type == TYPE_DEVICE) {
				if (this.getDevice().getName() == fileIns.getDevice().getName())
					return true;
			} else if (type == TYPE_CONTENT) {
				if (this.getContent().getTitle() == fileIns.getContent()
						.getTitle())
					return true;
			}
			return false;
	...
	private int checkContent(String fileName) {
		...
		for (int i = 0; i < total; i++) {
			if (mListContentName.get(i).getName().equals(fileName)) {
				return CONTENT_CHILD;
			}
		}

		if (mTraceFile.peek().getName().equals(fileName)) {
