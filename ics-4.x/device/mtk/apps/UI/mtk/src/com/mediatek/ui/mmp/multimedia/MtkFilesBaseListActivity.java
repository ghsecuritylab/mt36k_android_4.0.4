...
public class MtkFilesBaseListActivity extends FilesListActivity<FileAdapter> {
		...
		public void handleMessage(Message msg) {
			...
			case MSG_PHOTO_FRAME:
				if (mRecursiveFiles != null && mRecursiveFiles.size() > 0) {
					...
					Intent intent = new Intent(MtkFilesBaseListActivity.this,
	...
	protected int setCurrentSelection() {
		...
		// the absolute path
		if (sourceType == MultiFilesManager.SOURCE_SMB
				|| sourceType == MultiFilesManager.SOURCE_DLNA) {
	...
	protected void onNotSuppsort(String title) {
		...
		if (null == mTipsDialog) {
			...
			int y = (int) (display.getRawHeight() * 3 / 8) - 114 - height / 2;
...
//			destroyApp.finishAll();
			if (isValid()) {
				mAdapter.cancel();
				int state = Thumbnail.getInstance().getThumanilState();
				if (state != VideoConst.THUMANIL_STATUS_PREPARED) {
					Thumbnail.getInstance().resetThumbnail();
	...
	// Added by Dan for fix bug DTV00379203
	private void select() {
		if (!getListSelectedItem().isFile()) {
			return;
		}

		String selectedFile = getListSelectedItem().getAbsolutePath();
	...
	private void copy() {
		...
		String copyedFile = getListSelectedItem().getAbsolutePath();
		if (mCopyedFiles.size() > 0) {
			String oneCopyedFile = mCopyedFiles.get(0);
			...
			public void run() {
				try {
					for (String path : mCopyedFiles) {
						...
						List<FileAdapter> items = getListItems();
				...
				case FilesManager.REQUEST_REFRESH: {
					...
					if (mFilesManager instanceof MultiFilesManager) {
						if (((MultiFilesManager) mFilesManager)
							...
							if (mFilesManager.getFilesCount() == 0) {
								onListEmpty();
							}
						}
					}
