...
public class NewPipLogic {
	...
	private static final String START_ACTION = "com.mediatek.mediaplayer.start";
...
//				setTvViewPosition();
	...
	public boolean isCurrentModePIP() {
		return (mTvInputManager.getOuputMode() == TVInputCommon.OUTPUT_MODE_PIP);
	}
		...
		if (runningTaskList != null && runningTaskList.size() > 1) {
			...
			if ("com.mediatek.ui.TurnkeyUiMainActivity".equals(currentActivity
				...
				if (mBreComponentName) {
					...
					if (ct.isCurActivityTkuiMainActivity() && !isTvNormal()) {
						resumeMainOutput();
					}else{
						MtkLog.i(TAG,"~~~~~~~~~~~~"+ct.getCurRunningClass());
					}
				}
			} else {
	...
	//	mTvContent.setColorKey(true, 0xfff74dff);
		setScreenPos();
		setTvViewPosition();
		mTvContent.setScreenPosition("sub", x, y, w, h);
		}
