...
public class ComDialog extends Dialog implements OnItemSelectedListener {
	...
	private void setDialogParams() {
		...
		menuWidth = (int) (display.getRawWidth() * 0.15);
		menuHight = (int) (menuWidth * 4/ 3 + (int)(display.getRawHeight()*0.014));
		marginY = (int) (display.getRawHeight() * 3 / 8) - (int)(display.getRawHeight()*0.16)- menuHight / 2;
		marginX = (int) (display.getRawWidth() * 0.05);
		...
		lp.x = display.getRawWidth() / 2 - ((3 * menuWidth) / 2) - marginX;
		lp.y = marginY;
	...
	public void setWindowPosition() {
		...
		Window window = getWindow();
		WindowManager.LayoutParams lp = window.getAttributes();
		lp.x = display.getRawWidth() / 2 - menuWidth / 2 - marginX;
		lp.y = marginY;
		window.setAttributes(lp);
	}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		case KeyEvent.KEYCODE_DPAD_DOWN:
			if (position == PAGE_SIZE - 1) {
				...
				if (mSelectShow.equals(mDataList.get(getCount() - 1).content)) {
