...
public class MenuDataContainer {
	...
	// PQ Memo Type
	private String[] videoPQ_Memo_Type;
	...
	private DataItem fvBox;
	private DataItem fvBoxCvbsD;
	private DataItem fvBoxTvdC;
	private DataItem fvBoxCvbsS;
	private DataItem fvBoxSnowG;
	private DataItem fvBoxTvdG;
	private DataItem fvBoxVlockR;
	private DataItem factoyTV;
	private DataItem IPSettings;
	...
	public void loadDataVideo() {
		...
		// PQ Memo Type  
		videoPQ_Memo_Type = mContext.getResources().getStringArray(
				R.array.menu_video_pq_demo_type);
	...
	public void loadAdvancedData() {
		...
		if (TVInputManager.INPUT_TYPE_VGA.equals(mTV.getInputManager()
			...
			// PQ Memo FUNC
			ItemName = MenuConfigManager.PQDEMOFUNC;   
			DataItem mVideoPQMemoFunc = new DataItem(ItemName, mContext
					.getString(R.string.menu_video_pq_demo_func), 
					MenuConfigManager.INVALID_VALUE,
					MenuConfigManager.INVALID_VALUE,
					MenuConfigManager.INVALID_VALUE, null,
					MenuConfigManager.STEP_VALUE,
					DataItem.DataType.HAVESUBCHILD);
			mVideoPQMemoFunc.mSubChildGroup = new ArrayList<DataItem>();
			mVideoPQMemoFunc.setmParentGroup(mAdvancedVideo.mSubChildGroup);

			// PQ Memo Type
			ItemName = MenuConfigManager.PQDEMOTYPE;
			DataItem VideoPQMemoType = new DataItem(ItemName, mContext
					.getString(R.string.menu_video_pq_demo_type),
					MenuConfigManager.INVALID_VALUE,
					MenuConfigManager.INVALID_VALUE, mConfigManager
							.getDefault(ItemName)
							- mConfigManager.getMin(ItemName), videoPQ_Memo_Type,
					MenuConfigManager.STEP_VALUE,
					DataItem.DataType.SWICHOPTIONVIEW);

			mVideoPQMemoFunc.mSubChildGroup.add(VideoPQMemoType);
			VideoPQMemoType.setmParent(mVideoPQMemoFunc);

			//  PQ Memo Action  
			ItemName = MenuConfigManager.PQDEMOACTION; 
			DataItem VideoPQMemoAction = new DataItem(ItemName, mContext
					.getString(R.string.menu_video_pq_demo_action),
					MenuConfigManager.INVALID_VALUE,
					MenuConfigManager.INVALID_VALUE,
					MenuConfigManager.INVALID_VALUE, null,
					MenuConfigManager.STEP_VALUE,
					DataItem.DataType.HAVESUBCHILD);
			VideoPQMemoAction.mSubChildGroup = new ArrayList<DataItem>();
			VideoPQMemoAction.setmParent(mVideoPQMemoFunc);			
			mVideoPQMemoFunc.mSubChildGroup.add(VideoPQMemoAction);

			VideoPQMemoType.mEffectGroup = new ArrayList<DataItem>();
			VideoPQMemoType.mEffectGroup.add(VideoPQMemoAction);
			VideoPQMemoType.mSwitchHashMap = new HashMap<Integer, Boolean[]>();
			// off,not available
			VideoPQMemoType.mSwitchHashMap.put(0, new Boolean[] { false });
			VideoPQMemoType.mSwitchHashMap.put(1, new Boolean[] { true });
			VideoPQMemoType.mSwitchHashMap.put(2, new Boolean[] { true });
			VideoPQMemoType.mSwitchHashMap.put(3, new Boolean[] { true });
			
			if (VideoPQMemoType.mInitValue == 0) {
			//if (mConfigManager.getDefault(MenuConfigManager.PQ_DEMO_TYPE) == 0) {
				VideoPQMemoAction.setEnable(false);
			} else {
				VideoPQMemoAction.setEnable(true);
			}
			...
			mAdvancedVideo.mSubChildGroup.add(mVideoPQMemoFunc);
	...
	public void loadDataTV() {
		...
		DataItem tvChannel = new DataItem(ItemName, mContext
	...
	public void loadDataSetup() {
		...
		DataItem screenModeItem = new DataItem(ItemName, mContext
				...
				MenuConfigManager.STEP_VALUE, DataItem.DataType.OPTIONVIEW);
		if (!TVInputManager.INPUT_TYPE_AV.equals(TVContent
		...
		String[] net_interface = mContext.getResources().getStringArray(
				R.array.menu_setup_network_interface_array);
		...
		// Configuration
		DataItem ConfigItem = new DataItem("Configuration", mContext
				.getString(R.string.menu_setup_configuration),
				MenuConfigManager.INVALID_VALUE,
				MenuConfigManager.INVALID_VALUE,
				MenuConfigManager.INVALID_VALUE, null,
				MenuConfigManager.STEP_VALUE, DataItem.DataType.HAVESUBCHILD);
		ConfigItem.mSubChildGroup = new ArrayList<DataItem>();
		ConfigItem.setmParentGroup(network.mSubChildGroup);

		DataItem NetConnection = new DataItem(
				MenuConfigManager.NETWORK_CONNECTION, mContext
						.getString(R.string.menu_setup_internet_connection),
				MenuConfigManager.INVALID_VALUE,
				MenuConfigManager.INVALID_VALUE, saveV
						.readValue(MenuConfigManager.NETWORK_CONNECTION),
				net_connection, MenuConfigManager.STEP_VALUE,
				DataItem.DataType.SWICHOPTIONVIEW);

		// network interface type
		DataItem NetInterface = new DataItem(
				MenuConfigManager.NETWORK_INTERFACE, mContext
						.getString(R.string.menu_setup_interface),
				MenuConfigManager.INVALID_VALUE,
				MenuConfigManager.INVALID_VALUE, saveV
						.readValue(MenuConfigManager.NETWORK_INTERFACE),
				net_interface, MenuConfigManager.STEP_VALUE,
				DataItem.DataType.SWICHOPTIONVIEW);

		// wireless setting
		DataItem WirelessSetting = new DataItem("Wireless", mContext
				.getString(R.string.menu_setup_wireless_setting),
				MenuConfigManager.INVALID_VALUE,
				MenuConfigManager.INVALID_VALUE,
				MenuConfigManager.INVALID_VALUE, null,
				MenuConfigManager.STEP_VALUE, DataItem.DataType.HAVESUBCHILD);
		WirelessSetting.mSubChildGroup = new ArrayList<DataItem>();
		WirelessSetting.setmParentGroup(ConfigItem.mSubChildGroup);

		NetInterface.mEffectGroup = new ArrayList<DataItem>();
		NetInterface.mEffectGroup.add(WirelessSetting);

		NetInterface.mSwitchHashMap = new HashMap<Integer, Boolean[]>();

		// network information
		DataItem NetInformation = new DataItem(MenuConfigManager.NETWORK_INFO,
				mContext.getString(R.string.menu_setup_information),
				MenuConfigManager.INVALID_VALUE,
				MenuConfigManager.INVALID_VALUE,
				MenuConfigManager.INVALID_VALUE, null,
				MenuConfigManager.STEP_VALUE, DataItem.DataType.HAVESUBCHILD);
		NetInformation.mSubChildGroup = new ArrayList<DataItem>();
		NetInformation.setmParentGroup(ConfigItem.mSubChildGroup);

		// Ip settings
		IPSettings = new DataItem("IPSetting", mContext
				.getString(R.string.menu_setup_ip_setting),
				MenuConfigManager.INVALID_VALUE,
				MenuConfigManager.INVALID_VALUE,
				MenuConfigManager.INVALID_VALUE, null,
				MenuConfigManager.STEP_VALUE, DataItem.DataType.HAVESUBCHILD);
		IPSettings.mSubChildGroup = new ArrayList<DataItem>();
		IPSettings.setmParentGroup(ConfigItem.mSubChildGroup);

		NetInterface.mEffectGroup.add(IPSettings);

		NetInterface.mSwitchHashMap.put(0, new Boolean[] { false, true });
		NetInterface.mSwitchHashMap.put(1, new Boolean[] { true, false });
		if (saveV.readValue(MenuConfigManager.NETWORK_INTERFACE) == 0) {
			WirelessSetting.setEnable(false);
			IPSettings.setEnable(true);
		} else {
			WirelessSetting.setEnable(true);
			IPSettings.setEnable(false);
		}

		// connection test
		DataItem ConnectionTest = new DataItem("ConnectionTest", mContext
				.getString(R.string.menu_setup_connection_test),
				MenuConfigManager.INVALID_VALUE,
				MenuConfigManager.INVALID_VALUE,
				MenuConfigManager.INVALID_VALUE, null,
				MenuConfigManager.STEP_VALUE, DataItem.DataType.HAVESUBCHILD);
		ConnectionTest.mSubChildGroup = new ArrayList<DataItem>();
		ConnectionTest.setmParentGroup(ConfigItem.mSubChildGroup);

		// upgrade network
		NetConnection.mEffectGroup = new ArrayList<DataItem>();
		NetConnection.mEffectGroup.add(NetInterface);
		NetConnection.mEffectGroup.add(WirelessSetting);
		NetConnection.mEffectGroup.add(NetInformation);
		NetConnection.mEffectGroup.add(IPSettings);
		NetConnection.mEffectGroup.add(ConnectionTest);
		NetConnection.mSwitchHashMap = new HashMap<Integer, Boolean[]>();
		NetConnection.mSwitchHashMap.put(0, new Boolean[] { false, false,
				false, false, false });
		NetConnection.mSwitchHashMap.put(1, new Boolean[] { true, true, true,
				true, true });
		if (saveV.readValue(MenuConfigManager.NETWORK_CONNECTION) == 0) {
			NetInterface.setEnable(false);
			WirelessSetting.setEnable(false);
			NetInformation.setEnable(false);
			IPSettings.setEnable(false);
			ConnectionTest.setEnable(false);
		} else {
			if (saveV.readValue(MenuConfigManager.NETWORK_INTERFACE) == 0) {
				WirelessSetting.setEnable(false);
				IPSettings.setEnable(true);
			...
			NetInterface.setEnable(true);
			NetInformation.setEnable(true);			
			ConnectionTest.setEnable(true);
		}
		...
		ConfigItem.mSubChildGroup.add(NetConnection);
		ConfigItem.mSubChildGroup.add(NetInterface);
		ConfigItem.mSubChildGroup.add(WirelessSetting);
		ConfigItem.mSubChildGroup.add(NetInformation);
		ConfigItem.mSubChildGroup.add(IPSettings);
		ConfigItem.mSubChildGroup.add(ConnectionTest);
...
//		}
	 ...
	 * load ip setting
	 */
	public void loadIpSetting() {
		
		String strConMode = netWork.getConnectMode();
		int conMode = 0;
		if (strConMode.equals("dhcp")) {
			conMode = 0;
		} else if (strConMode.equals("manual")) {
			conMode = 1;
		}

		// IP address type
		ItemName = MenuConfigManager.ADDRESS_TYPE;
		DataItem AddressType = new DataItem(ItemName, mContext
				.getString(R.string.menu_setup_address_type),
				MenuConfigManager.INVALID_VALUE,
				MenuConfigManager.INVALID_VALUE, conMode, mContext
						.getResources().getStringArray(
								R.array.menu_setup_address_type_array),
				MenuConfigManager.STEP_VALUE, DataItem.DataType.SWICHOPTIONVIEW);
		// IP address
		String[] ipStr = getIPArray(netWork.getIPAddress());
		if (ipStr == null || ipStr[0].equals("0")) {
			ipStr = new String[] { "0", "0", "0", "0" };
		}
		DataItem IPAddress = new DataItem(MenuConfigManager.SETUP_IP_ADDRESS,
				mContext.getString(R.string.menu_setup_ip_address), 0, 100, 0,
				ipStr, MenuConfigManager.STEP_VALUE,
				DataItem.DataType.IPINPUTBOX);

		// subnet mask
		String[] subStr = getIPArray(netWork.getMaskAddress());
		if (subStr == null || subStr[0].equals("0")) {
			subStr = new String[] { "0", "0", "0", "0" };
		}
		DataItem SubNetMask = new DataItem(MenuConfigManager.SETUP_SUBNET,
				mContext.getString(R.string.menu_setup_subnetwork_mask), 0,
				100, 0, subStr, MenuConfigManager.STEP_VALUE,
				DataItem.DataType.IPINPUTBOX);

		// default gateway
		String[] gateStr = getIPArray(netWork.getRouteAddress());
		if (gateStr == null || gateStr[0].equals("0")) {
			gateStr = new String[] { "0", "0", "0", "0" };
		}
		DataItem DefaultGateway = new DataItem(
				MenuConfigManager.SETUP_DEFAULT_GATE, mContext
						.getString(R.string.menu_setup_default_gateway), 0,
				100, 0, gateStr, MenuConfigManager.STEP_VALUE,
				DataItem.DataType.IPINPUTBOX);

		// preferred DNS
		String[] dnsStr = getIPArray(netWork.getDnsAddress());
		if (dnsStr == null || dnsStr[0].equals("0")) {
			dnsStr = new String[] { "0", "0", "0", "0" };
		}
		
		DataItem PreferredDNS = new DataItem(MenuConfigManager.SETUP_IP_DNS,
				mContext.getString(R.string.menu_setup_first_dns), 0, 100, 0,
				dnsStr, MenuConfigManager.STEP_VALUE,
				DataItem.DataType.IPINPUTBOX);

		// secondary DNS
		String[] dnsStr2 = getIPArray(netWork.getDns2Address());
		if (dnsStr2 == null || dnsStr2[0].equals("0")) {
			dnsStr2 = new String[] { "0", "0", "0", "0" };
		}
		DataItem secondDNS = new DataItem(MenuConfigManager.SETUP_IP_DNS2,
				mContext.getString(R.string.menu_setup_standby_dns), 0, 100, 0,
				dnsStr2, MenuConfigManager.STEP_VALUE,
				DataItem.DataType.IPINPUTBOX);

		AddressType.mEffectGroup = new ArrayList<DataItem>();
		AddressType.mEffectGroup.add(IPAddress);
		AddressType.mEffectGroup.add(SubNetMask);
		AddressType.mEffectGroup.add(DefaultGateway);
		AddressType.mEffectGroup.add(PreferredDNS);
		AddressType.mEffectGroup.add(secondDNS);
		AddressType.mSwitchHashMap = new HashMap<Integer, Boolean[]>();
		AddressType.mSwitchHashMap.put(0, new Boolean[] { false, false, false,
				false, false });
		AddressType.mSwitchHashMap.put(1, new Boolean[] { true, true, true,
				true, true });
		if (conMode == 0) {
			IPAddress.setEnable(false);
			SubNetMask.setEnable(false);
			DefaultGateway.setEnable(false);
			PreferredDNS.setEnable(false);
			secondDNS.setEnable(false);
		} else {
			IPAddress.setEnable(true);
			SubNetMask.setEnable(true);
			DefaultGateway.setEnable(true);
			PreferredDNS.setEnable(true);
			secondDNS.setEnable(true);
		}

		IPSettings.mSubChildGroup = new ArrayList<DataItem>();
		IPSettings.mSubChildGroup.add(AddressType);
		IPSettings.mSubChildGroup.add(IPAddress);
		IPSettings.mSubChildGroup.add(SubNetMask);
		IPSettings.mSubChildGroup.add(DefaultGateway);
		IPSettings.mSubChildGroup.add(PreferredDNS);
		IPSettings.mSubChildGroup.add(secondDNS);
	...
	public void loadChannelData() {
		for (int i = 0; i < chNum; i++) {
			...
			int tvColorNum = 0;
			if (ch.getColorSystemOption() != null) {
	...
	public void loadFactoryData() {
		...
		String[] mFaBoxArray = mContext.getResources().getStringArray(R.array.menu_factory_tv_system_dbox_array);
		
		String[] mFaBoxSnowArray = mContext.getResources().getStringArray(R.array.menu_factory_tv_system_dbox_snow_array);
		...
		if (TVInputManager.INPUT_TYPE_TV.equals(mTV.getInputManager()
			...
			if(NavIntegration.getInstance(mContext).isCurrentSourceATVPlaying()){
			ItemName = MenuConfigManager.FACTORY_TV_DBOX;
			fvBox = new DataItem(ItemName, mContext
					.getString(R.string.menu_factory_TV_dbox),
					mConfigManager.getMin(ItemName),
					mConfigManager.getMax(ItemName),
					mConfigManager
					.getDefault(ItemName)
					- mConfigManager.getMin(ItemName), mFaBoxArray,
					MenuConfigManager.STEP_VALUE,
					DataItem.DataType.OPTIONVIEW);
			
			
			ItemName = MenuConfigManager.FACTORY_TV_DBOX_CVBS_DELAY;
			fvBoxCvbsD = new DataItem(ItemName, mContext
					.getString(R.string.menu_factory_TV_dbox_cvbs_delay),
					mConfigManager.getMin(ItemName),
					mConfigManager.getMax(ItemName),
					mConfigManager
					.getDefault(ItemName)
					- mConfigManager.getMin(ItemName), null,
					MenuConfigManager.STEP_VALUE,
					DataItem.DataType.NUMVIEW);
			
			
			ItemName = MenuConfigManager.FACTORY_TV_DBOX_TVD_CHECK;
			fvBoxTvdC = new DataItem(ItemName, mContext
					.getString(R.string.menu_factory_TV_dbox_tvd_check),
					mConfigManager.getMin(ItemName),
					mConfigManager.getMax(ItemName),
					mConfigManager
					.getDefault(ItemName)
			...
			ItemName = MenuConfigManager.FACTORY_TV_DBOX_CVBS_SWING;
			fvBoxCvbsS = new DataItem(ItemName, mContext
					.getString(R.string.menu_factory_TV_dbox_cvbs_swing),
					mConfigManager.getMin(ItemName),
					mConfigManager.getMax(ItemName),
					mConfigManager
					.getDefault(ItemName)
			...
			ItemName = MenuConfigManager.FACTORY_TV_DBOX_SNOW_GEN;
			fvBoxSnowG = new DataItem(ItemName, mContext
					.getString(R.string.menu_factory_TV_dbox_snow_gen),
					mConfigManager.getMin(ItemName),
					mConfigManager.getMax(ItemName),
					mConfigManager
					.getDefault(ItemName)
					- mConfigManager.getMin(ItemName), mFaBoxSnowArray,
					MenuConfigManager.STEP_VALUE,
					DataItem.DataType.OPTIONVIEW);
			
			
			ItemName = MenuConfigManager.FACTORY_TV_DBOX_TVD_GAIN;
			fvBoxTvdG = new DataItem(ItemName, mContext
					.getString(R.string.menu_factory_TV_dbox_tvd_gain),
					mConfigManager.getMin(ItemName),
					mConfigManager.getMax(ItemName),
					mConfigManager
					.getDefault(ItemName)
			...
			ItemName = MenuConfigManager.FACTORY_TV_DBOX_VLOCK_RATIO;
			fvBoxVlockR = new DataItem(ItemName, mContext
					.getString(R.string.menu_factory_TV_dbox_vlock_ratio),
					mConfigManager.getMin(ItemName),
					mConfigManager.getMax(ItemName),
					mConfigManager
					.getDefault(ItemName)
				...
				if(mConfigManager.getDefault(MenuConfigManager.FACTORY_TV_DBOX)==0){
				 fvBoxCvbsD.setEnable(false);
				 fvBoxTvdC.setEnable(false);
				 fvBoxCvbsS.setEnable(false);
				 fvBoxSnowG.setEnable(false);
				 fvBoxTvdG.setEnable(false);
				 fvBoxVlockR.setEnable(false);
				
			}else{
				
				 fvBoxCvbsD.setEnable(true);
				 fvBoxTvdC.setEnable(true);
				 fvBoxCvbsS.setEnable(true);
				 fvBoxSnowG.setEnable(true);
				 fvBoxTvdG.setEnable(true);
				 fvBoxVlockR.setEnable(true);
			}
			
			factoyTV.mSubChildGroup.add(fvBox);
			factoyTV.mSubChildGroup.add(fvBoxCvbsD);
			factoyTV.mSubChildGroup.add(fvBoxTvdC);
			factoyTV.mSubChildGroup.add(fvBoxCvbsS);
			factoyTV.mSubChildGroup.add(fvBoxSnowG);
			factoyTV.mSubChildGroup.add(fvBoxTvdG);
			factoyTV.mSubChildGroup.add(fvBoxVlockR);
			
			
				
			fvBox.setmParent(factoyTV);
			
			fvBox.setmParentGroup(factoyTV.mSubChildGroup);
			fvBoxCvbsD.setmParentGroup(factoyTV.mSubChildGroup);
			fvBoxTvdC.setmParentGroup(factoyTV.mSubChildGroup);
			fvBoxCvbsS.setmParentGroup(factoyTV.mSubChildGroup);
			fvBoxSnowG.setmParentGroup(factoyTV.mSubChildGroup);
			fvBoxTvdG.setmParentGroup(factoyTV.mSubChildGroup);
			fvBoxVlockR.setmParentGroup(factoyTV.mSubChildGroup);
