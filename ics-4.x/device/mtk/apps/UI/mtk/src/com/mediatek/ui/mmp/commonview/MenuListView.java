...
//			return	onMute();			 
	...
	public int getSelection(String content, List<MenuFatherObject> menuList) {
		...
		//end
			...
			int time = PhotoPlayActivity.getDelayedTime();
			if (time == PhotoPlayActivity.DELAYED_SHORT) {
				return 0;

			} else if (time == PhotoPlayActivity.DELAYED_MIDDLE) {
				return 1;

			} else if (time == PhotoPlayActivity.DELAYED_LONG) {
				return 2;
			...
			return LogicManager.getInstance(mContext).getFontSize();
		} else if (content.equals(mContext.getString(R.string.mmp_menu_style))) {
			return LogicManager.getInstance(mContext).getFontStyle();
		} else if (content.equals(mContext.getString(R.string.mmp_menu_color))) {
			return LogicManager.getInstance(mContext).getFontColor();
