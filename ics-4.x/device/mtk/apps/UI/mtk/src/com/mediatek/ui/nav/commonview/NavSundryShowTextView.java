	...
	public void setAndShowTextContent(boolean flag, int keyCode) {
		...
		case KeyMap.KEYCODE_MTKIR_ASPECT:
			...
			if (mni.isCurrentSourceBlocked()) {
				tmpflag = true;
			} else if (mni.iGetCurrentChannel() != null && mni
							.iGetCurrentChannel().isBlocked()) {
				tmpflag = true;
...
//				tmpflag = true;
		...
		case KeyMap.KEYCODE_MTKIR_FREEZE:
			boolean fflag = false;
			if (mni.isCurrentSourceBlocked()
					&& !mni.isCurrentSourceUserBlocked()) {
				fflag = true;
			} else if (((mni.iGetCurrentChannel() != null && mni
					.iGetCurrentChannel().isBlocked()) && !mni
					.isCurrentChannelUserBlocked())) {
				fflag = true;
			} else if (!mni.iCurrentInputSourceHasSignal()) {
				fflag = true;
			} else if (mni.isAudioScrambled()) {
