...
//import com.mediatek.tvcm.PalPreSetChannel;
...
//	private ArrayList<MountPoint> mDevice;
	...
	private MenuConfigManager mMenuCfgMngr;
	...
	public void presetChannels() {
		...
		if (hasAddListener == false) {
			mChannelsIsChangedListenner = new ChannelsChangedListener() {
				...
				public void onChanged() {
					if(mTVManager.getChannels() != null && mTVManager.getChannels().size() > 0) {
						System.out.println(testTag + " select svc after presetChannels");
						mTVManager.select(mTVManager.getChannels().get(0));
					} else {
						if (mTVManager.getChannels() != null) {
							System.out.println(testTag + " channels size: " + mTVManager.getChannels().size());
						} else {
							System.out.println(testTag + " channels == null");
						}
					}
				}
			};
			...
			if (null != mChannelsIsChangedListenner) {
				System.out.println(testTag + " add mChannelsIsChangedListenner");
				hasAddListener = true; 
				mTVManager.addChannelsChangedListener(mChannelsIsChangedListenner);
			} else {
				System.out.println(testTag + " fail: null == mChannelsIsChangedListenner");
			}
		} else {
			System.out.println(testTag + " has added Listener. Don't add again !");
		}

		
		System.out.println(testTag + " start preset all channels");
		...
		testThread1 = new Thread() {
			public void run() {
				if (mTVScanner != null) {
					System.out.println(testTag + " preset analog-channels");
					mTVScanner.preSetAnalogChannels(palPreSetChannel);
				} else {
					System.out.println(testTag + " fail: mTVScanner == null");
				}
			}
		};
		testThread1.start();
		...
		testThread2 = new Thread() {
			public void run() {
				if (mTVScanner != null) {
					System.out.println(testTag + " preset air digital-channels");
					mTVScanner.preSetAirDigitalChannels(dtmbPreSetChannel);
				} else {
					System.out.println(testTag + " fail: mTVScanner == null");
				}
			}
		...
		testThread3 = new Thread() {
			public void run() {
				if (mTVScanner != null) {
					System.out.println(testTag + " preset cable digital Channels");
					mTVScanner.preSetCableDigitalChannels(dvbcPreSetChannel);
				} else {
					System.out.println(testTag + " fail: mTVScanner == null");
				}
			}
