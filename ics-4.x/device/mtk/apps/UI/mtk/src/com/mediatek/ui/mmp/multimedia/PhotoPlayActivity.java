...
public class PhotoPlayActivity extends MediaPlayActivity {
	...
	public static final int DELAYED_LONG = 9000;
	
	public static final int DELAYED_MIDDLE = 7000;
	
	public static final int DELAYED_SHORT = 5000;
	...
	private static final int FRAME_ONE_PHOTO_MODE = 1;
	private static final int FRAME_ALL_PHOTO_MODE = 2;
	...
	private class SleepDialog extends Dialog {
		...
		private void setDialogPosition() {
			...
			lp.x = -(int) (display.getRawWidth() * 0.35);
		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_PLAY:
				...
				if(playMode == FRAME_ONE_PHOTO_MODE){
				   mImageManager.playCur();
				}
				else {
					mImageManager.autoPlay();
				}
				break;
			...
			case MESSAGE_HIDDLE_FRAME: {
				...
				if (isNotSupport == true && isPhotoActivityLiving == true) {
					try{
					onNotSuppsort(PhotoPlayActivity.this.getResources()
...
//				//mLogicManager.setPhotoDecodeFailureListener(mPhotoDecodeFailListener);
				...
				if(isPhotoActivityLiving == true){
					onNotSuppsort(PhotoPlayActivity.this.getResources().getString(
	...
	private void findView() {
		...
		params.width = display.getRawWidth();
		params.height = display.getRawHeight();
		vShowView.setLayoutParams(params);
		vShowView.setWindow(display);
...
//			 if (mLastPlayState && null != mControlView) {
//				 mControlView.play();
//			 }
			showController();
	...
	private OnPhotoDecodeListener mPhotoDecodeListener = new OnPhotoDecodeListener() {
		...
		/*	mHandler.post(new Runnable() {
				...
				public void run() {
					onNotSuppsort(PhotoPlayActivity.this.getResources()
							.getString(R.string.mmp_photo_type_notsupport));
//					vShowView.setType(ConstPhoto.DRAWBLACK);
					vShowView.setRes(null);
					vShowView.run();
				}
			});	*/	
	...
	private void initShowPhoto() {
		...
		mLogicManager.setRotateDigree(0);
		...
		if (playMode == NORMAL_MODE) {
			...
			new Thread(new Runnable() {
				public void run() {
					...
					mCurBitmap = mLogicManager.transfBitmap(mPhotoFramePath);
...
//	private boolean mLastPlayState;
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		case KeyMap.KEYCODE_MTKIR_RECORD: {
			...
			// Deleted by Dan for fix bug DTV00376577
//			if (null != mControlView) {
//				mLastPlayState = mControlView.isPalying();
//				mControlView.pause();
//			}
		...
		case KeyMap.KEYCODE_MTKIR_GREEN: {
			if (mControlView.isPalying()) {
				...
				if (mLogicManager.getCurrentPath(Const.FILTER_IMAGE).endsWith(".gif")){
					...
					oriention = mLogicManager.getRotateDigree();										
					if (oriention >= 360) {
						oriention = 0;
					}

					mLogicManager.setRotateDigree(oriention + 90);
					
					newOriention = mLogicManager.getRotateDigree();
...
//				menuDialog.setItemEnabled(4, !isNotSupport);
				...
				//Added by Shuming for fix CR DTV00406972
				if(isNotSupport||mCurBitmap == null){ 
