...
public class VideoView extends SurfaceView implements MediaPlayerControl{
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		 ...
		 * > width * mVideoHeight ) { //MtkLog.i("@@@",
		 * "image too tall, correcting"); height = width * mVideoHeight /
		 * mVideoWidth; } else if ( mVideoWidth * height < width * mVideoHeight
		 * ) { //MtkLog.i("@@@", "image too wide, correcting"); width = height *
		 * mVideoWidth / mVideoHeight; } else { //MtkLog.i("@@@",
		 * "aspect ratio is correct: " + //width+"/"+height+"="+
		 * //mVideoWidth+"/"+mVideoHeight); } }
		 */
		// MtkLog.i("@@@@@@@@@@", "setting size: " + width + 'x' + height);
...
//		mContext.sendBroadcast(i);
		...
		try {
			...
			mIsPrepared = false;
			MtkLog.v(TAG, "reset duration to -1 in openVideo");
			mDuration = -1;
	...
	MediaPlayer.OnPreparedListener mPreparedListener = new MediaPlayer.OnPreparedListener() {
		public void onPrepared(MediaPlayer mp) {
			...
			mVideoHeight = mp.getVideoHeight();
			if (mVideoWidth != 0 && mVideoHeight != 0) {
				// MtkLog.i("@@@@", "video size: " + mVideoWidth +"/"+
				// mVideoHeight);
				MtkLog.d(TAG, "mSurfaceWidth-----------------");
				getHolder().setFixedSize(mVideoWidth, mVideoHeight);
	...
	private MediaPlayer.OnErrorListener mErrorListener = new MediaPlayer.OnErrorListener() {
		public boolean onError(MediaPlayer mp, int framework_err, int impl_err) {
			MtkLog.d(TAG, "Error: " + framework_err + "," + impl_err);
			if (mMediaController != null) {
				mMediaController.hide();
			}
