	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (mGroup != null && mGroup.size() > 0) {
			...
			if (dataItem.getDataType() == DataItem.DataType.NUMVIEW) {
				...
				if (!isEnabled(position)) {
					hodler.mNumView.getNameView().setTextColor(
							Color.GRAY);
					hodler.mNumView.getmShowView().setTextColor(
							Color.GRAY);
				}
				...
				hodler.mNumView
						.setValueChangedListener(new OnValueChangedListener() {
							public void onValueChanged(View v, int value) {
								...
								}else if(selId.startsWith(MenuConfigManager.FACTORY_TV_DBOX)){
...
//								}
			...
			if (dataItem.getDataType() == DataItem.DataType.IPINPUTBOX) {
				if (hodler.mIPInputBox == null) {
					hodler.mIPInputBox = new IPInputBox(mContext);
				}
				SaveValue saveValue = SaveValue.getInstance(mContext);							
				hodler.mIPInputBox.setAdapter(dataItem);
				hodler.mIPInputBox.setmId(dataItem.mItemID);
				if (!isEnabled(position)) {
					hodler.mIPInputBox.getmTextViewName().setTextColor(
							Color.GRAY);
					hodler.mIPInputBox.getmEditText1().setText("");
					hodler.mIPInputBox.getmEditText2().setText("");
					hodler.mIPInputBox.getmEditText3().setText("");
					hodler.mIPInputBox.getmEditText4().setText("");					
				} 
				hodler.mIPInputBox.setOnTouchListener(onTouchListener);
				return hodler.mIPInputBox;
			}
		...
		public boolean onTouch(View v, MotionEvent event) {
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				...
				if (!enabled) {
					...
					} else if (className.endsWith("IPInputBox")) {
						((MenuMain) mContext)
								.setIPInputBoxBlockUPDownKeyEvent(false);
						((MenuMain) mContext)
								.setListViewItemSelector(R.drawable.tk_cm_item_selected);
						((MenuMain) mContext).clearAllIPInput();
	...
	public static class DataItem {
		...
		public DataItem(String mItemID, String mName, int mStartValue,
			...
			if (mDataType == DataType.IPINPUTBOX) {
				this.mOptionValue = mOptionVaule;
			}
		}

		public enum DataType {
			OPTIONVIEW, POSITIONVIEW, PROGRESSBAR, HAVESUBCHILD, INPUTBOX, EFFECTOPTIONVIEW, SWICHOPTIONVIEW, CHANNELLISTVIEW, TEXTCOMMVIEW, SCANCHANNELSOPTIONVIEW, NUMVIEW, NUMADJUSTVIEW, IPINPUTBOX, DATETIMEVIEW, CHANNELPOWERONVIEW, PASSWORDVIEW, FACTORYOPTIONVIEW, FACTORYPROGRESSVIEW, TVSOURCEVIEW, CHANNELPOWERNOCAHNNEL
		}
	...
	void setBroadcastUTC() {
		if (SaveValue.getInstance(mContext).readValue(
			...
			SystemClock
					.setCurrentTimeMillis(mBroadcastUTC + mBroadcastTimeZone);
			SystemClock.setCurrentTimeMillis(mBroadcastUTC);
		}
