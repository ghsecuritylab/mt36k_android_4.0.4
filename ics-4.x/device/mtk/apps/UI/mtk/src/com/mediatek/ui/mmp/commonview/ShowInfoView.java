...
public class ShowInfoView extends Dialog implements OnDismissListener {
	...
	public void setWindowPosition() {
		...
		if (null != d) {
			menuWidth = d.getIntrinsicWidth()*(display.getRawWidth()/1280);
			menuHight = d.getIntrinsicHeight()*(display.getRawHeight()/720);
		}

		marginX = (int) (display.getRawWidth() * 0.15);

		lp.width = menuWidth;
		lp.height = menuHight;

		lp.x = display.getRawWidth() / 2 - menuWidth / 2 - marginX;
		lp.y = (int) (display.getRawHeight() * 3 / 8) - (int)(display.getRawHeight()*0.16) - menuHight / 2;
	...
	public void setAudioView() {
		...
		// TODO have chnage
		audioDuration.setText(formatText(formatTime((int) mLogicManager
