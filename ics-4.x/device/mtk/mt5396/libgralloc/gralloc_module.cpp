...
//	LOGI("gralloc_lock hnd->format is %d", hnd->format);	
	if(hnd->format == HAL_PIXEL_FORMAT_GTV_VIDEO_HOLE ||
		hnd->format == HAL_PIXEL_FORMAT_GTV_OPAQUE_BLACK || hnd->format == HAL_PIXEL_FORMAT_GTV_CMPB_VIDEO_HOLE)
	{
		LOGI("format IS HAL_PIXEL_FORMAT_GTV_VIDEO_HOLE or HAL_PIXEL_FORMAT_GTV_OPAQUE_BLACK");
		return -EINVAL;
	}
	if (hnd->flags & private_handle_t::PRIV_FLAGS_USES_UMP)
	{
