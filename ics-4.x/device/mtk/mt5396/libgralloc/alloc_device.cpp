...
#define GRALLOC_PITCH_ALIGN   //20120502 for pitch align issue
#ifdef GRALLOC_PITCH_ALIGN
...
static int alloc_device_alloc(alloc_device_t* dev, int w, int h, int format, int usage, buffer_handle_t* pHandle, int* pStride)
{
		  ...
		  || format == HAL_PIXEL_FORMAT_YCbCr_420_P ) 
	{
		switch (format)
		{
			...
			case HAL_PIXEL_FORMAT_YV12:
				#ifdef GRALLOC_PITCH_ALIGN
			 	stride = GRALLOC_ALIGN(w, 32);    //32 byte ALIGNED
			 	size = GRALLOC_ALIGN(h, 32)*(stride + GRALLOC_ALIGN(stride/2, 32));
				#else
				stride = (w + 15) & ~15; //DWORD ALIGNED
	...
	else
	{
		...
		case HAL_PIXEL_FORMAT_RGBA_4444:
			bpp = 2;
			break;
		case HAL_PIXEL_FORMAT_GTV_VIDEO_HOLE:
	...
	if (private_handle_t::validate(*pHandle) < 0)
	{
		return -EINVAL;
	}
