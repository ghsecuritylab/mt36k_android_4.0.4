...
#define _MT85XX_AUDIO
#define _USE_PCM_WRITE
#define _CHECK_CAPTURE_DEVICE_CAPABILITY
...
#ifndef _MT85XX_AUDIO
#include "ril_interface.h"
#endif

#ifdef _MT85XX_AUDIO
#ifdef _MT53XX_AUDIO
#define MT85XX_PLAYBACK_PERIOD_SIZE_IN_FRAMES    1024
#define MT85XX_PLAYBACK_PERIOD_COUNT             4
#define MT85XX_PLAYBACK_START_THRESHOLD          1
//#define MT85XX_WRITE_THRESHOLD_IN_FRAMES         1024
#define MT85XX_WRITE_THRESHOLD_IN_FRAMES         2048
#else
#define MT85XX_PLAYBACK_PERIOD_SIZE_IN_FRAMES    1024
#define MT85XX_PLAYBACK_PERIOD_COUNT             16
#define MT85XX_PLAYBACK_START_THRESHOLD          1
#define MT85XX_WRITE_THRESHOLD_IN_FRAMES         2048
#endif
#define MT85XX_USB_AUDIO_CARD_INDEX              1
#define MT85XX_USB_AUDIO_DEVICE_INDEX            0
#endif
...
#ifdef _MT85XX_AUDIO
    .period_size = MT85XX_PLAYBACK_PERIOD_SIZE_IN_FRAMES,
    .period_count = MT85XX_PLAYBACK_PERIOD_COUNT,
    #ifdef _MT53XX_AUDIO
    .format = PCM_FORMAT_S16_LE,
    #else
    .format = PCM_FORMAT_S16_BE,
    #endif
#else
...
#define AUDIO_OUTPUT_DEV_NAME_EXPECT 	"mtk - mtk"
#define AUDIO_OUTPUT_DEV_NAME_EXCLUDE	NULL
#define AUDIO_INPUT_DEV_NAME_EXPECT 	NULL
#define AUDIO_INPUT_DEV_NAME_EXCLUDE	"mtk - mtk"
...
static int start_output_stream(struct tuna_stream_out *out)
{
...
#ifdef _MT53XX_AUDIO	
	unsigned int card = get_sound_device_by_card(AUDIO_OUTPUT_DEV_NAME_EXPECT, AUDIO_OUTPUT_DEV_NAME_EXCLUDE, SND_CARD_PLAYBACK);
#else
...
#ifdef _MT85XX_AUDIO
#ifdef _MT53XX_AUDIO
	//[DTV00422699], for some unknown reason, the device can not correctly recognized, use the default value instead of -1 as card index.
	if (card == 0xFFFFFFFF)
	{
		card = 0;
	}
     ...
     */
#ifdef _MT85XX_AUDIO
    out->write_threshold = MT85XX_WRITE_THRESHOLD_IN_FRAMES;
    out->config.start_threshold = MT85XX_PLAYBACK_START_THRESHOLD;
    out->config.avail_min = MT85XX_PLAYBACK_PERIOD_SIZE_IN_FRAMES;
#else
...
static size_t out_get_buffer_size(const struct audio_stream *stream)
{
    ...
    be a multiple of 16 frames */
 #ifdef _MT85XX_AUDIO
    size_t size = (MT85XX_WRITE_THRESHOLD_IN_FRAMES * DEFAULT_OUT_SAMPLING_RATE) / out->config.rate;
 #else
...
static uint32_t out_get_latency(const struct audio_stream_out *stream)
{
    struct tuna_stream_out *out = (struct tuna_stream_out *)stream;
#ifdef _MT85XX_AUDIO
    return (MT85XX_WRITE_THRESHOLD_IN_FRAMES * 1000) / out->config.rate;
#else
                         ...
                         size_t bytes)
{
...
#ifdef DEBUG_AUDIO_DUMP
    {
        ...
        property_get("debug.audio.hw01", value, "0");
        if (atoi(value)) {
            int fd;
            char fileName[PATH_MAX];
    
            LOGI("line %d: hw01::write %d\n", __LINE__, bytes);
            // dump audio data
            snprintf(fileName, PATH_MAX, "/tmp/hw01.pcm");
            fd = open(fileName, O_WRONLY | O_CREAT | O_APPEND);
            if (fd != -1) {
                LOGI("line %d: debug.audio.hw01::write %d\n", __LINE__, bytes);
...
#ifdef _MT85XX_AUDIO	
#ifdef _MT53XX_AUDIO	
	unsigned int card = get_sound_device_by_card(AUDIO_INPUT_DEV_NAME_EXPECT, AUDIO_INPUT_DEV_NAME_EXCLUDE, SND_CARD_CAPTURE);
#else
    unsigned int card = MT85XX_USB_AUDIO_CARD_INDEX;
#endif
    unsigned int port = MT85XX_USB_AUDIO_DEVICE_INDEX;
#endif
...
#if (defined(_MT85XX_AUDIO) && defined(_CHECK_CAPTURE_DEVICE_CAPABILITY))
    {
        struct pcm_hw_info hw_info;
        memset(&hw_info, 0, sizeof(hw_info));
        // get hw info
        ret = pcm_get_hw_info(card, port, PCM_IN, &hw_info);
        LOGV("adev_open_input_stream pcm_get_hw_info ret = %d ch_max = %u ch_min = %u rate_max = %u rate_min = %u",
             ret, hw_info.channels_max, hw_info.channels_min, hw_info.rate_max, hw_info.rate_min);
        LOGV("adev_open_input_stream config.rate = %u requested_rate = %u config.channels = %u",
             in->config.rate, in->requested_rate, in->config.channels);
        // check capture hw's capability
        if (ret == 0) {
            // check channel count
            if (in->config.channels > hw_info.channels_max) {
                LOGD("adev_open_input_stream reject config.channels %u > channels_max %u", in->config.channels, hw_info.channels_max);
                *channel_mask = (hw_info.channels_max == 2) ? AUDIO_CHANNEL_IN_STEREO : AUDIO_CHANNEL_IN_MONO;
                ret = -EINVAL;
                goto err;
            }
            if (in->config.channels < hw_info.channels_min) {
                LOGD("adev_open_input_stream reject config.channels %u < channels_min %u", in->config.channels, hw_info.channels_min);
                *channel_mask = (hw_info.channels_min == 2) ? AUDIO_CHANNEL_IN_STEREO : AUDIO_CHANNEL_IN_MONO;
                ret = -EINVAL;
                goto err;
            }
            // check sample rate
            if (in->config.rate > hw_info.rate_max) {
                LOGD("adev_open_input_stream change config.rate from %u to %u", in->config.rate, hw_info.rate_max);
                in->config.rate = hw_info.rate_max;
            }
            if (in->config.rate < hw_info.rate_min) {
                LOGD("adev_open_input_stream change config.rate from %u to %u", in->config.rate, hw_info.rate_min);
                in->config.rate = hw_info.rate_min;
            }
        }
    }
#endif
...
#ifdef _MT85XX_AUDIO
#ifdef _MT53XX_AUDIO
        .name = "MT5396 audio HW HAL",
#else
        .name = "MT8580 audio HW HAL",
#endif
#else
