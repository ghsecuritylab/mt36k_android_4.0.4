...
include $(word 1, $(subst /vm_linux/,/vm_linux /, $(shell pwd)))/dtv_linux/mak/gmsl-1.0.12/gmsl
include $(word 1, $(subst /vm_linux/,/vm_linux /, $(shell pwd)))/dtv_linux/mak/gmd-1.0.2/gmd
endif
...
subdir_makefiles := \
	$(shell \
	if [ ! -e $(OUT_DIR)/root_subdir_makefiles.mak ]; then \
		build/tools/findleaves.py --prune=out --prune=.repo --prune=.git $(subdirs) Android.mk > $(OUT_DIR)/root_subdir_makefiles.mak ; \
	fi; \
		cat $(OUT_DIR)/root_subdir_makefiles.mak \
	)
...
$(warning ALL_PREBUILT=$(ALL_PREBUILT))
...
.PHONY: clean
clean:
	@if [ -e build/core/fclean.sh ]; then \
		sh build/core/fclean.sh $(OUT_DIR);\
	else \
		rm -rf $(OUT_DIR); \
	fi
