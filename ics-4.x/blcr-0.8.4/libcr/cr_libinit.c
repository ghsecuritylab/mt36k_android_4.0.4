 ...
 *
 * $Id: //DTV/MP_BR/DTV_X_IDTV0801_002158_10_001_158_001/android/ics-4.x/blcr-0.8.4/libcr/cr_libinit.c#1 $
 */
...
static int cri_sigaction(int signum, const struct sigaction *act, struct sigaction *oact) {
    ...
    if (rc != 0) {
        printf("Please change rt_sigaction parameter to meet kernel requirement. By liyi.li\n");
    }
...
static void __attribute__((constructor)) cri_init(void)
{
    ...
    //
    //dlopen can't open NULL(itself) under android platfrom. fixed pls. liyi
#ifndef LIYI
    ...
    #if 0 // Cannot use a GLIBC_PRIVATE symbol w/ RPMS
	    __nss_disable_nscd();
    #else
