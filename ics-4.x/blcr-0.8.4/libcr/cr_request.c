 ...
 *
 * $Id: //DTV/MP_BR/DTV_X_IDTV0801_002158_10_001_158_001/android/ics-4.x/blcr-0.8.4/libcr/cr_request.c#1 $
 *
                          ...
                          cr_checkpoint_handle_t *handle)
{
	...
	// XXX: Only support a single version at the moment
        LOGE("Checkpoint args version mismatch, %d used, but %d expected.", args->cr_version, CR_CHECKPOINT_ARGS_VERSION);
        goto out;
    }
    ...
    if (token < 0) {
        LOGE("fail to initialize the handle");
        goto out;
    }
    ...
    if (rc < 0) {
        LOGE("Checkpoint fail. Error number:%d, rc=%d", errno, rc);
