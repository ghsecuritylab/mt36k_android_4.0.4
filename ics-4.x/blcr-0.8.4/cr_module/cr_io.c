 ...
 *
 * $Id: //DTV/MP_BR/DTV_X_IDTV0801_002158_10_001_158_001/android/ics-4.x/blcr-0.8.4/cr_module/cr_io.c#1 $
 */
...
static struct dentry *cr_lookup_create(struct nameidata *nd, int is_dir)
{
    ...
    if (IS_ERR(dentry)) {
      cr_inode_unlock(nd->path.dentry->d_inode);
    }
    return dentry;
}
...
cr_dentry_open(struct dentry *dentry, struct vfsmount *mnt, int flags)
{
    ...
    filp = ERR_PTR(err);
    if (!IS_ERR(filp))
        filp = dentry_open(dentry, mnt, flags, cr_current_cred());
...
cr_mknod(cr_errbuf_t *eb, struct nameidata *nd, const char *name, int mode, unsigned long unlinked_id)
{
	...
	err = kern_path(name, LOOKUP_FOLLOW, &nd->path);
	if (!err) {
    ...
    switch (mode & S_IFMT) {
    case S_IFREG:
	err = vfs_create(nd->path.dentry->d_inode, dentry, mode, nd);
	break;
    case S_IFIFO:
	err = cr_vfs_mknod(nd->path.dentry->d_inode, dentry, nd->nd_mnt, mode, 0 /* ignored */);
	break;
    default:
	...
	(void)cr_vfs_unlink(nd->path.dentry->d_inode, dentry, nd->nd_mnt);
	dput(dentry);
    }
    cr_inode_unlock(nd->path.dentry->d_inode);
...
cr_link(cr_errbuf_t *eb, struct path *old_path, const char *name)
{
     ...
     */
	retval = kern_path(name, LOOKUP_FOLLOW, &nd.path);
    if (retval < 0) {
	CR_ERR_EB(eb, "cr_link: path_lookup(%s) returned %d", name, retval);
	goto out_free;
    }
    new_dentry = dget(nd.path.dentry);
    cr_path_release(&nd);
