 ...
 *
 * $Id: //DTV/MP_BR/DTV_X_IDTV0801_002158_10_001_158_001/android/ics-4.x/blcr-0.8.4/cr_module/cr_rstrt_req.c#1 $
 */
...
alloc_rstrt_req(void)
{
	...
	req->errbuf = cr_errbuf_alloc();
	{
	    ...
	    if (tmp && !FD_ISSET(0, fdt->close_on_exec)) {
		req->dpipe_in = tmp;
		get_file(tmp);
	    }
	    /* Dup stdout, unless marked CLOEXEC */
	    tmp = fcheck_files(files, 1);
	    if (tmp && !FD_ISSET(1, fdt->close_on_exec)) {
		req->dpipe_out = tmp;
		get_file(tmp);
	    }
	    rcu_read_unlock();
...
cr_restore_open_chr(cr_rstrt_proc_req_t *proc_req, struct cr_file_info *file_info, int do_not_restore)
{
    ...
    /* XXX:  Validate type */
    retval = -EINVAL;
    if ((int)cf_chrdev.cr_type != (int)cr_open_chr) {
...
cr_restore_open_dup(cr_rstrt_proc_req_t *proc_req, struct cr_file_info *file_info, int do_not_restore)
{
    ...
    retval = -EINVAL;
    if ((int)cf_dup.cr_type != (int)cr_open_dup) {
...
cr_chdir(const char *path)
{
	...
	/* lookup the path */
	retval = kern_path(path, LOOKUP_FOLLOW|LOOKUP_DIRECTORY, &nd.path);
	if (retval)
        goto out;
