 ...
 *
 * $Id: //DTV/MP_BR/DTV_X_IDTV0801_002158_10_001_158_001/android/ics-4.x/blcr-0.8.4/vmadump4/vmadump_common.c#1 $
 *
...
int load_map(cr_rstrt_proc_req_t *ctx,
	...
	return 0;//vmad_load_arch_map(ctx, file, head);
#else
...
// add by zhanghanzhi
static inline unsigned long is_mmu_nonmapped(unsigned long x)
{
...
int addr_nonzero_file(struct mm_struct *mm, unsigned long addr) {
    ...
    // add by zhanghangzhi
    // Don't check non-mapped page and return 0 directly
    if (is_mmu_nonmapped(addr)) return 0;
    // end
