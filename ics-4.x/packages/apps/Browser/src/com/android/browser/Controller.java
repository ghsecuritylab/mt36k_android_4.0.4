...
public class Controller
    ...
    protected void onPause() {
        ...
        Log.e(LOGTAG, "Got onPause() callback, mtkCachePolicy = %d" + mtkCachePolicy);

        if(1 == mtkCachePolicy){
            for (int pos = 0; pos < mTabControl.getTabCount(); pos++) {
                tab = mTabControl.getTab(pos);
                if (tab != null){
                    tab.getWebView().freeMemory();
                }
            }
        }
        // -- MTK_WEBKIT_FIX END --
    ...
    public void updateMenuState(Tab tab, Menu menu) {
        ...
        // -- MTK_WEBKIT_FIX START --
        final MenuItem freeAllAvailableMemory = menu.findItem(R.id.free_all_available_memory_menu_id);
        freeAllAvailableMemory.setVisible(showDebugSettings);
        freeAllAvailableMemory.setEnabled(showDebugSettings);
        // -- MTK_WEBKIT_FIX END --
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.free_all_available_memory_menu_id:
                for (int pos = 0; pos < mTabControl.getTabCount(); pos++) {
                    Tab tab = mTabControl.getTab(pos);
                    if (tab != null){
                        tab.getWebView().freeMemory();
                    }
                }
