...
public class BrowserSettings implements OnSharedPreferenceChangeListener,
    ...
    // [CR: DTV00390141]
    // Insert the "AppleCoreMedia" keyword to the iPad UA so we can download the video data
    // from Sohu news (http://news.sohu.com/)
    private static final String IPAD_USERAGENT = "Mozilla/5.0 AppleCoreMedia/1.0.0.8J2 (iPad; U; " +
        "CPU OS 4_3like Mac OS X; en-us) AppleWebKit/533.17.9 " +
        "(KHTML, like Gecko) Version/4.0.4 Mobile/7B367 Safari/6533.18.5";
    ...
    private void syncStaticSettings(WebSettings settings) {
        ...
        settings.setProperty(WebViewProperties.gfxUseMinimalMemory, (1 == mtkCachePolicy) ? "true" : "false");

        Log.v("browserSetting", "Set use minimal memory to " + ((1 == mtkCachePolicy) ? "true" : "false"));
        // -- MTK_WEBKIT_FIX END --
    ...
    public int getUserAgent() {
        ...
        // Enable the UA selection even the debug option is not turned on
        /*
        if (!isDebugEnabled()) {
            return 0;
        }
    ...
    public void disablePluginIfNeed(String url, WebView view) {
        ...
        Log.v("browserSetting", "url = " + url + ", PluginState is " + settings.getPluginState());
    }
    ...
    public void adjustUAStringIfNeed(String url, WebView view) {
        ...
        Log.v("browserSetting", "url = " + url + ", Current UA String is " + settings.getUserAgentString());
    }
