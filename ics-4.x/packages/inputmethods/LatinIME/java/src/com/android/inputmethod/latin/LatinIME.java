...
public class LatinIME extends InputMethodServiceCompatWrapper implements KeyboardActionListener,
	...
	private static boolean sugestionShow;
    ...
    public static class UIHandler extends StaticInnerHandlerWrapper<LatinIME> {
        ...
        public void postUpdateSuggestions() {
			...
			Log.v("onKeyDown","----------------postUpdateSuggestions----------------");
        ...
        public void startDisplayLanguageOnSpacebar(boolean localeChanged) {
			Log.v("onKeyDown","----------------startDisplayLanguageOnSpacebar----------------");
            final LatinIME latinIme = getOuterInstance();
        ...
        public void onStartInputView(EditorInfo attribute, boolean restarting) {
			Log.v("onKeyDown","---------------------------------------------onStartInputView----------------");
        ...
        public void onFinishInputView(boolean finishingInput) {
			Log.v("onKeyDown","----------------onFinishInputView----------------");
            if (hasMessages(MSG_PENDING_IMS_CALLBACK)) {
...
//add
    ...
    /* package private */ void resetSuggestMainDict() {
		Log.v("onKeyDown","----------------resetSuggestMainDict----------------");
        final String localeStr = mSubtypeSwitcher.getInputLocaleStr();
        final Locale keyboardLocale = LocaleUtils.constructLocaleFromString(localeStr);
    ...
    Log.v("onKeyDown","----------------onConfigurationChanged----------------");
    ...
    Log.v("onKeyDown","----------------onCreateInputView----------------");

		keyboardView = mKeyboardSwitcher.getKeyboardView();
        return mKeyboardSwitcher.onCreateInputView();
    }

    @Override
    public void setInputView(View view) {
    Log.v("onKeyDown","----------------setInputView----------------");
    ...
    Log.v("onKeyDown","----------------setCandidatesView----------------");
    ...
    private void onStartInputInternal(EditorInfo attribute, boolean restarting) {
		...
		Log.v("onKeyDown","----------------onStartInputInternal----------------");
    ...
    private void onStartInputViewInternal(EditorInfo attribute, boolean restarting) {
		...
		Log.v("onKeyDown","----------------onStartInputViewInternal----------------");
    ...
    private void initializeInputAttributes(EditorInfo attribute) {
		Log.v("onKeyDown","----------------initializeInputAttributes----------------");
        if (attribute == null)
            return;
    ...
    Log.v("onKeyDown","----------------onWindowHidden----------------");
    ...
    private void onFinishInputInternal() {
		Log.v("onKeyDown","----------------onFinishInputInternal----------------");
        super.onFinishInput();
    ...
    private void onFinishInputViewInternal(boolean finishingInput) {
		Log.v("onKeyDown","----------------onFinishInputViewInternal----------------");
    ...
    Log.v("onKeyDown","----------------onUpdateExtractedText----------------");
    ...
    public void onUpdateSelection(int oldSelStart, int oldSelEnd,
			...
			Log.v("onKeyDown","----------------onUpdateSelection----------------");
    ...
    public void setLastSelection(int start, int end) {
		Log.v("onKeyDown","----------------setLastSelection----------------");
    ...
    Log.v("onKeyDown","----------------onExtractedTextClicked----------------");
    ...
    Log.v("onKeyDown","----------------onExtractedCursorMovement----------------"+isSuggestionsRequested());
        if (isSuggestionsRequested()) return;
    ...
    Log.v("onKeyDown","----------------updateFullscreenMode----------------"+isFullscreenMode());
...
/*
    ...
    */
			...
			Log.v("onKeyDown","------------onKeyDown--------------------"+keyCode);

			keyboardView = mKeyboardSwitcher.getKeyboardView();

			Log.v("onKeyDown","------------keyboardView--------------------"+keyboardView);
						...
						*/
						...
						if (nLastKeyIndex >= nCurKeyboardKeyNums - 1) {
							...
							int[] nearestKeyIndices = nCurrentKeyboard.getNearestKeys(
									nKeys.get(nLastKeyIndex).mX, nKeys.get(nLastKeyIndex).mY);
					...
					case KeyEvent.KEYCODE_DPAD_CENTER:
						...
						*/
						...
						if (null == keyboardView){
							Log.v("onKeyDown","-------------null == keyboardView-------------------");
							 return false;
							}
						...
						//onPress(curKeyCode, false);

						Log.v("onKeyDown","-------------curKeyCode-------------------"+curKeyCode+"---Keyboard.CODE_PERIOD---"+Keyboard.CODE_PERIOD+"---Keyboard.CODE_DASH---"+Keyboard.CODE_DASH);
						
						switch (curKeyCode) {							
							case Keyboard.CODE_ENTER:
							case Keyboard.CODE_TAB:
							case Keyboard.CODE_SPACE:
							//case Keyboard.CODE_PERIOD:
							//case Keyboard.CODE_DASH:
							//case Keyboard.CODE_SINGLE_QUOTE:
							//case Keyboard.CODE_DOUBLE_QUOTE:
							// TODO: Check how this should work for right-to-left languages. It seems to stand
							// that for rtl languages, a closing parenthesis is a left parenthesis. Is this
							// managed by the font? Or is it a different char?
							case Keyboard.CODE_CLOSING_PARENTHESIS:
							case Keyboard.CODE_CLOSING_SQUARE_BRACKET:
							case Keyboard.CODE_CLOSING_CURLY_BRACKET :
							case Keyboard.CODE_CLOSING_ANGLE_BRACKET:
							case Keyboard.CODE_DIGIT0:
							case Keyboard.CODE_PLUS :
							
							
							/** Special keys code.	These should be aligned with values/keycodes.xml */
							case Keyboard.CODE_DUMMY:
							...
							case Keyboard.CODE_UNSPECIFIED :
							...
							//case 10:
								...
								onCodeInput(curKeyCode, null, -1, -1);
							...
							default:
								...
								//if (mKeyboardSwitcher.isAlphabetMode() /*&& keyboardView.isShifted()*/) {
								...
								Log.v("onKeyDown","-------------mSettingsValues.isWordSeparator(curKeyCode)--------------"+mSettingsValues.isWordSeparator(curKeyCode));
								if (mSettingsValues.isWordSeparator(curKeyCode) && mKeyboardSwitcher.isShiftedOrShiftLocked()) {

										if(curKeyCode == 47){
												if(sugestionShow){
													sendKeyChar((char)(curKeyCode+17));	
												 } else if(32 == nKeys.get( (nLastKeyIndex + 1) ).mCode){
													sendKeyChar((char)(curKeyCode+17));	
												 }else{
													sendKeyChar((char)(curKeyCode+11));	
													}												
											}else if(curKeyCode == 44){
											sendKeyChar((char)(curKeyCode-11));
											}else if(curKeyCode == 58){
														onTextInput(":-)");
											}else if (curKeyCode == 39){
														sendKeyChar((char)(curKeyCode - 5));
											}else if (curKeyCode == 64){
														sendKeyChar((char)curKeyCode);
											}else if(curKeyCode == 46){
													if(32 == nKeys.get( (nLastKeyIndex + 1) ).mCode){
														onTextInput(".com");
														}else{
															sendKeyChar((char)(curKeyCode+17));
														}																				
            					...
            					}else if(mKeyboardSwitcher.isShiftedOrShiftLocked()){
										if(curKeyCode == 45){
															sendKeyChar((char)(curKeyCode+50));
										}else if (curKeyCode == 39){
													sendKeyChar((char)(curKeyCode - 5));
										}else if(curKeyCode == 58){
													onTextInput(":-)");
									 	...
									 	if(curKeyCode == 46){
												if(32 == nKeys.get( (nLastKeyIndex + 1) ).mCode){
															onTextInput(".com");
												}else if(mSettingsValues.isWordSeparator(curKeyCode)){
															sendKeyChar((char)(curKeyCode));
														}																				
												}else if(curKeyCode == 58){
													onTextInput(":-)");
								...
								Log.v("onKeyDown","-------------curKeyCode-----default--------------"+curKeyCode);
								//onCodeInput(curKeyCode, null, -1, -1);																						
								break;
    ...
    private void onSettingsKeyPressed() {
		Log.v("onKeyDown","----------------onSettingsKeyPressed----------------");
    ...
    Log.v("onKeyDown","----------------onCustomRequest----------------");
    ...
    private boolean isShowingOptionDialog() {
		Log.v("onKeyDown","----------------isShowingOptionDialog----------------");
    ...
    Log.v("onKeyDown","----------------onCodeInput---------11-------"+x+"--"+y);
        ...
        case Keyboard.CODE_TAB:
            ...
            // focus but will never move the . Or, we send a real tab keycode, which some
    ...
    private void handleBackspace(boolean justReplacedDoubleSpace) {
		Log.v("onKeyDown","----------------handleBackspace----------------");
    ...
    public boolean isShowingSuggestionsStrip() {
		Log.v("onKeyDown","----------------isShowingSuggestionsStrip----------------");
    ...
    public void switchToKeyboardView() {
		Log.v("onKeyDown","----------------switchToKeyboardView----------------");
        if (DEBUG) {
