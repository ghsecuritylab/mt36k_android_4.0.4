...
public class LatinKeyboardView extends KeyboardView implements PointerTracker.KeyEventHandler,
	...
	public void onDraw(Canvas canvas) {
	    ...
	    Log.v("onDraw","-------------in-----------------"+focusedKey);
		super.onDraw(canvas);
		currentKeyboard = this.getKeyboard();
		keys = currentKeyboard.getKeys();
		Log.v("onDraw","--------------keys.size()----------------"+keys.size());
		Paint p = new Paint();
		p.setColor(Color.CYAN);
		p.setStyle(Paint.Style.STROKE);
		p.setStrokeWidth(3.75f);
		focusedKey = keys.get(lastKeyIndex);
		rect = new Rect(focusedKey.mX, focusedKey.mY, focusedKey.mX
				+ focusedKey.mWidth, focusedKey.mY + focusedKey.mHeight);
		canvas.drawRect(rect, p);
		Log.v("onDraw","--------------out----------------"+focusedKey);
	}
