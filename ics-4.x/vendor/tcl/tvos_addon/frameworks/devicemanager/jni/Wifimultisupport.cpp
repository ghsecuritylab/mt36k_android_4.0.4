...
{
	LOGE(" ########### wifi_get_deviceNum #########");
	int num = 0;
	num = find_existing_devices();
	LOGE("find_existing_devices exit !! %d\n", num);
...
int set_wifi_switch(int state)
{
	...
	if(NULL == T_set_wifi_switch) {
		LOGE("there is no symbol named set_wifi_switch, function: %s line: %d",
				__FUNCTION__, __LINE__);
		//exit(EXIT_FAILURE);
		return -1;
	}
...
int get_wifi_switch()
{
	...
	if(NULL == T_get_wifi_switch) {
		LOGE("there is no symbol named get_wifi_switch, function: %s line: %d",
				__FUNCTION__, __LINE__);
		//exit(EXIT_FAILURE);
		return 0;
	}
...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    if (registerMethods(env) != 0) {
        LOGE("ERROR: Wifimultisupport native registration failed\n");
        goto bail;
    }
