 ...
 */
		...
		try {
			...
			*/		
			...
			/*
			private TStorageVolume(String path, String description, boolean removable,
            boolean emulated, int mtpReserveSpace, int storageId,
            boolean allowMassStorage, long maxFileSize) 
			*/
			result[i] = new TStorageVolume(tsv.getPath(), tsv.getDescription(), tsv.isRemovable(),
			tsv.isEmulated(),tsv.getMtpReserveSpace() , tsv.getStorageId(), 
			tsv.allowMassStorage(), tsv.getMaxFileSize() );
 ...
 */
    public String getVolumeLabel(String mountPoint) {
        ...
        if (mMountService == null) return new String();
        try {
            return mMountService.getVolumeLabel(mountPoint);
        } catch (RemoteException e) {
            Log.e(TAG, "Failed to get volume state", e); 
            return null;
        }   
