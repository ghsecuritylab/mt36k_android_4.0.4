...
static inline int tag_index(int tag){
		if(tag == PTT_SRV_NAME)
			{return TAG_SRV_NAME;}
		else if(tag == PTT_AC_NAME)
			{return TAG_AC_NAME;}
		else if(tag == PTT_HOST_UNIQ)
			{return TAG_HOST_UNIQ;}
		else if(tag == PTT_AC_COOKIE)
			{return TAG_AC_COOKIE;}
		else if(tag == PTT_VENDOR)
			{return TAG_VENDOR;}
		else if(tag == PTT_RELAY_SID)
			{return TAG_RELAY_SID;}
		else if(tag == PTT_SRV_ERR)
			{return TAG_SRV_ERR;}
		else if(tag == PTT_SYS_ERR)
			{return TAG_SYS_ERR;}
		else if(tag == PTT_GEN_ERR)
			{return TAG_GEN_ERR;}
		else if(tag == PTT_EOL)
			{return TAG_EOL;}
		else return -1;
...
static int tag_map[] = { PTT_SRV_NAME,
    ...
    for(;i<MAX_TAGS;++i){
    	switch(i)
    	{
    		case 0:    			buf[i] = get_tag(ph,PTT_SRV_NAME);    			break;
    		case 1:    			buf[i] = get_tag(ph,PTT_AC_NAME);    			break;
    		case 2:    			buf[i] = get_tag(ph,PTT_HOST_UNIQ);    			break;
    		case 3:    			buf[i] = get_tag(ph,PTT_AC_COOKIE);    			break;
    		case 4:    			buf[i] = get_tag(ph,PTT_VENDOR);    			break;
    		case 5:    			buf[i] = get_tag(ph,PTT_RELAY_SID);    			break;
    		case 6:    			buf[i] = get_tag(ph,PTT_SRV_ERR);    			break;
    		case 7:    			buf[i] = get_tag(ph,PTT_SYS_ERR);    			break;
    		case 8:    			buf[i] = get_tag(ph,PTT_GEN_ERR);    			break;
    		case 9:    			buf[i] = get_tag(ph,PTT_EOL);    			break;
    		default: 				break;
    		}
...
struct pppoe_tag *make_filter_tag(short type, short length, char* data){
    ...
    fatal("make_filter_tag over===");//add by HQS
    return pt;
}
