...
#endif
#ifdef MTM_HWDEC
static BOOL mtimage_jpg_dec_close(void)
{
...
BOOL mtmplay_jpg_dec(j_decompress_ptr cinfo)
{
    ...
    case JCS_RGB:
    case JCS_RGBA_8888:
        rMtimgSetting.eColorFormat = 0; //GPU_PIXEL_FORMAT_ARGB
        break;
    case JCS_RGB_565:
        rMtimgSetting.eColorFormat = 3;//GPU_PIXEL_FORMAT_RGB16
        break;
    default:
    ...
    fgStop = FALSE;
    MTIMAGE_SetStopFlag(HW_JPG_DEC, fgStop);
    MTIMAGE_Connect(HW_JPG_DEC, TRUE, rMtimgSetting.eColorFormat);
    ...
    if (!fgImgRetErr)
    {
        ...
        SkJpegDebugf("***0. output_width = %d, output_height = %d\n", cinfo->output_width,  cinfo->output_height);
#ifdef JPGMEM_IMPROVE
        cinfo->input_iMCU_row = 	(JDIMENSION)u4Buf ;
#endif
    }
    //careful, should be release state
	mtimage_jpg_dec_close();
