...
static size_t out_get_buffer_size(const struct audio_stream *stream)
{
    ...
    return out->buffer_size; //out->buffer_size * (BUF_NUM_PERIODS / 2);
#else
                         ...
                         size_t bytes)
{
...
#ifdef DEBUG_AUDIO_DUMP
    {
            ...
            property_get("debug.audio.a2dp01", value, "0");
            if (atoi(value)) 
            {
                int fd;
                char fileName[PATH_MAX];
    
                LOGI("line %d: a2dp01::write %d\n", __LINE__, bytes);
                // dump audio data
                snprintf(fileName, PATH_MAX, "/tmp/a2dp01.pcm");
                fd = open(fileName, O_WRONLY | O_CREAT | O_APPEND);
                if (fd != -1) {
                    LOGI("line %d: debug.audio.a2dp01::write %d\n", __LINE__, bytes);
...
#ifdef OMAP_ENHANCEMENT
    out->buffer_size = 512 * 2; //512 * 10;
#else
    out->buffer_size = 512 * 2; //512 * 20;
#endif
