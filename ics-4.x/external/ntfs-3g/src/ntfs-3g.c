...
static int ntfs_fuse_read(const char *org_path, char *buf, size_t size,
		loff_t offset, struct fuse_file_info *fi __attribute__((unused)))
{
	ntfs_inode *ni = NULL;
	ntfs_attr *na = NULL;
	char *path = NULL;
...
static int ntfs_fuse_trunc(const char *org_path, loff_t size,
#if !KERNELPERMS | (POSIXACLS & !KERNELACLS)
			BOOL chkwrite)
#else
...
static int ntfs_fuse_truncate(const char *org_path, loff_t size)
{
	return ntfs_fuse_trunc(org_path, size, TRUE);
}

static int ntfs_fuse_ftruncate(const char *org_path, loff_t size,
			struct fuse_file_info *fi __attribute__((unused)))
{
	/*
