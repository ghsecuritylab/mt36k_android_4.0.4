...
BOOL ntfs_boot_sector_is_ntfs(NTFS_BOOT_SECTOR *b)
{
	...
	ntfs_log_error("Beginning bootsector check.\n");

	ntfs_log_error("Checking OEMid, NTFS signature.\n");
	if (b->oem_id != cpu_to_le64(0x202020205346544eULL)) { /* "NTFS    " */
		ntfs_log_error("NTFS signature is missing.\n");
		goto not_ntfs;
	}

	ntfs_log_error("Checking bytes per sector %d .\n", b->bpb.bytes_per_sector);
	...
	ntfs_log_error("Checking sectors per cluster. %d \n", b->bpb.sectors_per_cluster);
	...
	ntfs_log_error("Checking cluster size.\n");
	...
	ntfs_log_error("Checking reserved fields are zero.\n");
	...
	ntfs_log_error("Checking clusters per mft record.\n");
	...
	ntfs_log_error("Checking clusters per index block.\n");
	...
	if (b->end_of_sector_marker != cpu_to_le16(0xaa55))
		ntfs_log_error("Warning: Bootsector has invalid end of sector "
			       "marker.\n");
...
int ntfs_boot_sector_parse(ntfs_volume *vol, const NTFS_BOOT_SECTOR *bs)
{
	...
	ntfs_log_error("SectorSize = 0x%x\n", vol->sector_size);
	ntfs_log_error("SectorSizeBits = %u\n", vol->sector_size_bits);
	/*
	...
	ntfs_log_error("SectorsPerCluster = 0x%x\n", sectors_per_cluster);
	...
	ntfs_log_error("NumberOfSectors = %lld\n", (long long)sectors);
	if (!sectors) {
	...
	//lseek(....., SEEK_END);
	//ltell(...);
	ntfs_log_perror("seek dev: %x, offset: %x .\n", vol->dev, (long long)((sectors - 1) << vol->sector_size_bits));
	...
	ntfs_log_error("MFT LCN = %lld\n", (long long)vol->mft_lcn);
	ntfs_log_error("MFTMirr LCN = %lld\n", (long long)vol->mftmirr_lcn);
