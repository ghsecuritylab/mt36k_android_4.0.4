			 ...
			 int *vol_id)
{
	...
	sprintf(file, lib->ubi_vol, i, _minor - 1);
	fd = open(file, O_RDONLY);
	if (fd == -1) {
		errno = ENODEV;
		return -1;
...
static int mtd_node_to_num(const char *mtd_dev_node)
{
	...
	_major = major(sb.st_rdev);
	_minor = minor(sb.st_rdev);

	if (_major != MTD_CHAR_MAJOR) {
		errno = EINVAL;
...
int ubi_detach_mtd_scan(libubi_t desc, const char *node, int mtd_num)
{
	int ret, ubi_dev;
	...
	return ubi_remove_dev_scan(desc, node, ubi_dev);
}
...
int ubi_detach_scan(libubi_t desc, const char *node, const char *mtd_dev_node)
{
	int mtd_num;
	...
	return ubi_detach_mtd_scan(desc, node, mtd_num);
}
...
int ubi_remove_dev_scan(libubi_t desc, const char *node, int ubi_dev)
{
	int fd, ret;
	...
	if (fd == -1)
		return sys_errmsg("cannot open \"%s\"", node);
	ret = ioctl(fd, UBI_IOCDETSCN, &ubi_dev);
	if (ret == -1)
		goto out_close;
...
out_not_ubi:
	errmsg("\"%s\" has major:minor %d:%d, but this does not correspond to "
	       "any existing UBI device or volume", node, _major, _minor);
	errno = ENODEV;
	return -1;
}
