...
"-s, --scan                      scan head info and lzo the table, force detach\n"
...
static const struct option long_options[] = {
	...
	{ .name = "scan",     .has_arg = 0, .flag = NULL, .val = 's' },
...
static int parse_opt(int argc, char * const argv[])
{
	while (1) {
		...
		key = getopt_long(argc, argv, "p:m:d:shV", long_options, NULL);
		if (key == -1)
			break;
...
int main(int argc, char * const argv[])
{
	...
	if (args.devn != -1) {
		if (args.scan != -1) {
			err = ubi_remove_dev_scan(libubi, args.node, args.devn);
		} else {
		...
		if (args.dev != NULL) {
			if (args.scan != -1) {
				err = ubi_detach_scan(libubi, args.node, args.dev);
			} else {
			...
			if (args.scan != -1) {
				err = ubi_detach_mtd_scan(libubi, args.node, args.mtdn);
			} else {
