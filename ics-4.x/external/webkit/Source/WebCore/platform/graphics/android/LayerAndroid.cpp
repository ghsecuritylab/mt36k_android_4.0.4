...
/// M: for zValue synchronize @{
void LayerAndroid::updateDrawingZValue()
{
    android::AutoMutex lock(m_atomicSync);
    m_drawingZValue = m_zValue;
}
/// @}
...
/// M: for zValue Syncrhonize
static inline bool compareLayerDrawZ(const LayerAndroid* a, const LayerAndroid* b)
{
    return a->drawingZValue() > b->drawingZValue();
}
/// @}
...
bool LayerAndroid::drawChildrenCanvas(SkCanvas* canvas)
{
    ...
    if (count > 0) {
        Vector <LayerAndroid*> sublayers;
        for (int i = 0; i < count; i++) {
            LayerAndroid* child = this->getChild(i);
            child->updateDrawingZValue();
            sublayers.append(child);
            //sublayers.append(this->getChild(i));
        }

        // now we sort for the transparency
        std::stable_sort(sublayers.begin(), sublayers.end(), compareLayerDrawZ);
        for (int i = 0; i < count; i++) {
