				      ...
				      struct wpa_scan_results *scan_res)
{
	...
	wpa_printf(MSG_INFO, "P2P: Scan results received (%d BSS)",
		   (int) scan_res->num);
			 ...
			 const u8 *req_dev_types)
{
	...
	wpa_printf(MSG_INFO, "P2P: wpa_s->scan_res_handler = wpas_p2p_scan_res_handler");
				   ...
				   struct wpa_scan_results *scan_res)
{
	...
	wpa_printf(MSG_INFO, "P2P: Scan results received (%d BSS) for join",
...
static void wpas_p2p_join_scan(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_printf(MSG_INFO, "P2P: wpa_s->scan_res_handler = wpas_p2p_scan_res_join");
