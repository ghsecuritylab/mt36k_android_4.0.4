...
bool BmpDecoderHelper::DecodeImage(SkStream* stream,
  ...
  uint8* pui1_bmp_buffer = new uint8[BMP_BUFFER_SIZE];
  if (pui1_bmp_buffer == NULL)
  {
  	return false;
  }
  memset(pui1_bmp_buffer, 0, BMP_BUFFER_SIZE);
  buffer_len_ = stream->read(pui1_bmp_buffer, BMP_BUFFER_SIZE);
  buffer_idx_ = 0;
  
  data_ = pui1_bmp_buffer;
  io_stream_ = stream;
  pos_ = 0;
  len_ = len;
  inverted_ = true;
...
uint8 BmpDecoderHelper::GetByte() {
  ...
  {
  	buffer_len_ = io_stream_->read(data_, BMP_BUFFER_SIZE);
	buffer_idx_ = 0;
  }

  if (buffer_idx_ >= BMP_BUFFER_SIZE)
  {
  	return 0;
  }
  
  return data_[buffer_idx_++];
 
}

void BmpDecoderHelper::BackBytes(uint32 bytes)
{
	io_stream_->rewind();
	for(int i =0; i <(pos_ - bytes); i++)
	{
		io_stream_->readU8();
	}
...
void BmpDecoderHelper::SkipBytes(uint32 bytes)
{
	io_stream_->rewind();
	for(int i =0; i <(pos_ + bytes); i++)
	{
