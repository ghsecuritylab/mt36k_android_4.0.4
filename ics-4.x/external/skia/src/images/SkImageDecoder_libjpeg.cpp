...
bool SkJPEGImageDecoder::onDecode(SkStream* stream, SkBitmap* bm, Mode mode) {
    ...
    J_DITHER_MODE dithermode;

    cinfo.err = jpeg_std_error(&sk_err);
    sk_err.error_exit = skjpeg_de_error_exit;
...
SkJpegDebugf("$$$onDecode jpeg_create_decompress\n");
    ...
    */
    ...
    dithermode = cinfo.dither_mode;
    cinfo.fgforceswmode = FALSE;
    ...
    */
 #ifdef TIME_DECODE
    AutoTimeMillis atm2("JPEG Hw Decode");
#endif
	//SkDebugf("Ondecode jpeg_start_decompress %d\n",cinfo.out_color_space);
...
#ifdef TIME_DECODE
    atm2.get();
    AutoTimeMillis atm3("JPEG Readscanline");
#endif
SkJpegDebugf("$$$onDecode recompute_sampleSize\n");    
...
//SkDebugf("1 allocPixelRef width = %d,height = %d, color mode = %d\n",cinfo.output_width, cinfo.output_height,cinfo.out_color_space);
    ...
    //    sc = SkScaledBitmapSampler::kRGB_565;
          SkJpegDebugf("#####Set SkScaledBitmapSampler to kRGB_565\n");
#endif
...
// SkDebugf("2 allocPixelRef width = %d,height = %d, color mode = %d\n",sampler.scaledWidth(),sampler.scaledHeight(),cinfo.out_color_space);
        if (!this->allocPixelRef(bm, NULL)) {
            return return_false(cinfo, *bm, "allocPixelRef");
        }
    } else if (SkImageDecoder::kDecodeBounds_Mode == mode) {
	...
	SkJpegDebugf("------------------- bm2 size %d [%d %d] %d\n", bm->getSize(), bm->width(), bm->height(), bm->config());

//    SkDebugf("------------------- bm2 size %d [%d %d] %d\n", bm->getSize(), bm->width(), bm->height(), bm->config());
    return true;
}

bool SkJPEGImageDecoder::stopDecode(void){
	SkDebugf("------------Stop decode jpeg\n");
	jpeg_hw_stop_decode();
	return true;
}
...
SkJpegDebugf("$$$ SkJPEGImageEncoder\n");
