...
static boolean sk_resync_to_restart(j_decompress_ptr cinfo, int desired) {
    ...
    src->next_input_byte = (const JOCTET*)src->fBuffer;
    src->bytes_in_buffer = src->fStream->getLength();
	
    //SkDebugf("xxxxxxxxxxxxxx sk_resync_to_restart to rewind src->bytes_in_buffer %d\n",src->bytes_in_buffer);
    return TRUE;
}
...
//SkDebugf("**************** use memorybase %x %d \n", baseAddr, stream->getLength());
}
