...
void Semaphore::timedwait(long lSeconds, int lNSeconds)
{
    int ret;
    struct timespec to;

    to.tv_sec = time(NULL) + lSeconds;
    to.tv_nsec = lNSeconds;
    
    if((ret = pthread_cond_timedwait(&cond, &mutex_, &to)) != 0)
    {
        // wait fail.
        LOG(0, "TimedWait fail, %d\n", ret);
    }
}
