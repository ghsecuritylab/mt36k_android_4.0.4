...
/** 
 * Scoped Mutex wrapper. Uses "Resource Acquisition is 
 * Initialization" idiom to implement the Scoped Locking.
 */
class ScopedMutex{
public:
    ScopedMutex(Mutex &mutex):mutex_(mutex){
        mutex_.lock();
    }
    ~ScopedMutex(){
        mutex_.unlock();
...
private:
    ...
    ScopedMutex const& operator=(ScopedMutex const&);
    Mutex &mutex_;
};
