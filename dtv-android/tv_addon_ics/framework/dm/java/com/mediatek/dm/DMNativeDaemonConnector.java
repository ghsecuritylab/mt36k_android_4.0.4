...
final class DMNativeDaemonConnector implements Runnable {
    ...
    DMNativeDaemonConnector(IDMNativeDaemonConnectorCallbacks callbacks,
    ...
    private void listenToSocket() throws IOException {
        ...
        Slog.i(TAG, "Start to listenToSocket.\n");
        try {
                    ...
                    if (buffer[i] == 0) {
                        ...
                        try {
                            ...
                            if (code >= ResponseCode.UnsolicitedInformational) {
                                try {
                                    if (!mCallbacks.onEvent(code, temp, tokens)) {
    ...
    private void sendCommand(String command, String argument) {
        synchronized (this) {
            if (LOCAL_LOGD) {
                if(argument != null){
                    Slog.d(TAG, String.format("SND -> {%s} {%s}", command, argument));
                }else{
                Slog.d(TAG, String.format("SND -> {%s}", command));
                }
            }
     ...
     */
    public  void doCommand(String cmd)  //synchronized ArrayList<String>
            throws DMNativeDaemonConnectorException  {
        sendCommand(cmd);
        ...
        while (!complete) {
            try {
                ...
                try {
                    ...
                    throw new DMNativeDaemonConnectorException(
                            String.format("Invalid response from daemon (%s)", line));
                }

                if ((code >= 200) && (code < 801)) {
                    complete = true;
                }
                response.add(line);
            } catch (InterruptedException ex) {
    ...
    /*public String[] doListCommand(String cmd, int expectedResponseCode)
        ...
        for (int i = 0; i < rsp.size(); i++) {
            ...
            try {
                ...
                } else if (code == DMNativeDaemonConnector.ResponseCode.CommandOkay) {
        ...
        throw new DMNativeDaemonConnectorException("Got an empty response");
    }
    */
}
